function report=airdropx_wind_airdrop_mission_v131(projectRoot,opts)
%AIRDROPX_WIND_AIRDROP_MISSION_V131 Wind-disturbance-aware Physics-MPC + precision airdrop.
%
...
if opts.OutputRoot=="", opts.OutputRoot=fullfile(projectRoot,"matlab","results","physics_mpc_v131_wind_disturbance_airdrop",opts.ScenarioName+"_"+mode); end
...
try
    ...
    for k=1:Nsim
        ...
        % Release guidance. Truth wind is prohibited here. v1.3.1 keeps the
        ...
        if cfg<4
            ...
            if SR.release_now
                ...
                release.wind_est_mps(j)=WindEst(k); release.wind_rate_est_mps2(j)=WindRate(k); release.wind_truth_mps(j)=WindTrue(k); release.predicted_impact_m(j)=SR.estimated_impact_at_release_m;
                ...
                fprintf("[V131 DROP] %s %s cargo%d t=%.3f phase=0 target=%.2f release=%.2f pred=%.2f truth=%.2f miss=%+.2f windEst=%+.2f truthWind=%+.2f\n", ...
        ...
        if scheduledRelease
            ...
            tRel=t(k)+tau;
            wEstRel=WindEst(k)+WindRate(k)*tau; bp=airdropx_airdrop_ballistic_params_v121(); wEstRel=max(min(wEstRel,bp.max_abs_forecast_wind_mps),-bp.max_abs_forecast_wind_mps);
            ...
            release.wind_est_mps(j)=wEstRel; release.wind_rate_est_mps2(j)=WindRate(k); release.wind_truth_mps(j)=double(DRel.wind_long_mps);
            ...
            relPredState=struct("x_m",release.release_x_m(j),"h_m",release.release_h_m(j),"vx_ground_mps",release.release_vg_mps(j),"vz_up_mps",release.release_vz_mps(j),"wind_est_mps",wEstRel,"wind_rate_est_mps2",WindRate(k));
            ...
            fprintf("[V131 DROP] %s %s cargo%d t=%.3f phase=%.3f target=%.2f release=%.2f pred=%.2f truth=%.2f miss=%+.2f windEst=%+.2f truthWind=%+.2f\n", ...
        ...
        if k==1 || DropEvent(k) || mod(k,50)==0
            fprintf("[V131] %s %s t=%5.1f cfg=%d x=%.1f hErr=%+.3f VaErr=%+.3f wind=%+.2f/%+.2f dW1=%+.3f res=%.3g qp=%.2fms\n", ...
    ...
    T=table(t,Cfg,PosN,X(:,1),X(:,2),X(:,3),X(:,4),X(:,5),Vg,Vz,WindTrue,WindRaw,WindEst,WindRate,WindSigma,ForecastDw1,ResidualNorm,WindMpcActive,CurrentTarget,PredImpact,DropEvent,ReleaseIndex,U(:,1),U(:,2),QPExit,QPTime,PredErrInf,primarySeries,causalMask,ReleasePhase_s,FractionalRelease,Mass,Cg,Iyy, ...
    ...
    report=struct("version","Physics-MPC v1.3.1 wind-disturbance-aware precision airdrop","pass",gate.pass,"scenario",opts.ScenarioName,"mode",mode,"control_mode",controlMode,"release_mode",releaseMode,"oracle_version",vOracle,"options",opts,"metrics",metrics,"gate",gate,"cargo",Cargo,"gust_event_indices",gustEvents,"completed_at",datetime("now"));
...
fprintf(fid,"Physics-MPC v1.3.1 wind-disturbance-aware precision airdrop\nscenario=%s\nmode=%s\npass=%d\n",r.scenario,r.mode,r.pass);
...
try
    f=figure("Visible","off","Position",[80 80 1550 980]); tl=tiledlayout(f,2,2,"TileSpacing","compact","Padding","compact"); title(tl,"AirdropX v1.3.1 "+scenario+" / "+mode);
