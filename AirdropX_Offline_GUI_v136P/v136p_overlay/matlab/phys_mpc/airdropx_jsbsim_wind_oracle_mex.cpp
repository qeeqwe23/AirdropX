...
public:
    ...
    mxArray* startContinuous(const std::vector<double>& x, const std::vector<double>& u,
                             int cfgId, double fuelScale, double windLongMps)
    {
        validateCfgFuelTime(cfgId, fuelScale, dt_);
        if (!std::isfinite(windLongMps)) throw std::runtime_error("windLongMps must be finite.");
        prepareState(x, u, cfgId, fuelScale, false);
        setLongitudinalWind(windLongMps);
        // Refresh the algebraic force/derivative loop at the same physical state.
        // No translational/rotational integration occurs here.
        settleAndPrimeCurrentDynamics();
        continuousStarted_ = true;
        continuousCfg_ = cfgId;
        continuousFuelScale_ = fuelScale;
        continuousWindMps_ = windLongMps;
        continuousPosNM_ = 0.0;
        continuousTimeS_ = 0.0;
        return continuousDiagnostics(0.0, cfgId);
    }

    std::pair<std::vector<double>, mxArray*> stepContinuous(const std::vector<double>& u,
                                                              int cfgId, double fuelScale,
                                                              double windLongMps, double Ts)
    {
        if (!continuousStarted_) throw std::runtime_error("start_continuous must be called before step_continuous.");
        validateCfgFuelTime(cfgId, fuelScale, Ts);
        if (!std::isfinite(windLongMps)) throw std::runtime_error("windLongMps must be finite.");
        if (cfgId < continuousCfg_) throw std::runtime_error("Continuous payload cfg cannot decrease.");
        if (cfgId != continuousCfg_ || std::abs(fuelScale-continuousFuelScale_) > 1e-12) {
            applyMassConfiguration(cfgId, fuelScale);
            verifyMassConfiguration(cfgId, fuelScale);
            // v1.3.5: JSBSim's derived inertia/cg properties can lag a direct
            // point-mass write by one model pass.  This matters when a fractional
            // payload release leaves only one 1/120-s integration substep in the
            // current 0.1-s MPC hold.  Refresh the algebraic mass properties with
            // integration frozen BEFORE the first physical substep at the new cfg.
            refreshMassPropertiesZeroDt();
            verifyMassConfiguration(cfgId, fuelScale);
            continuousCfg_ = cfgId;
            continuousFuelScale_ = fuelScale;
        }
        ...
        verifyControlEcho(u);
        const int nSteps = stepsFor(Ts);
        const double pos0 = continuousPosNM_;
        for (int k=0; k<nSteps; ++k) {
            // Match the Simulink plant semantics: the commanded wind is held
            // throughout every JSBSim base-rate substep.
            setLongitudinalWind(windLongMps);
            if (!fdm_->Run()) throw std::runtime_error("JSBSim Run failed during continuous wind step.");
            const double vn = get("velocities/v-north-fps", 0.0)*kFtToM;
            if (!std::isfinite(vn)) throw std::runtime_error("Non-finite north groundspeed during continuous wind step.");
            continuousPosNM_ += vn*dt_;
            continuousTimeS_ += dt_;
        }
        return {readState(readN1(baseN1_),readN2(baseN2_)), continuousDiagnostics(continuousPosNM_-pos0, cfgId)};
    }
    ...
    mxArray* info() const
    {
        ...
        mxSetField(s,0,"version",mxCreateString("AirdropX JSBSim wind-continuous oracle v1.2.1+mass-refresh-v135"));
        return s;
    }
...
private:
    ...
    void refreshMassPropertiesZeroDt()
    {
        if (fdm_->IntegrationSuspended())
            throw std::runtime_error("Mass-property refresh entered with integration already suspended.");
        constexpr int kMaxPasses=12;
        constexpr int kMinPasses=2;
        constexpr double kTol=1.0e-12;
        lastMassRefreshIterations_=0;
        lastMassRefreshError_=std::numeric_limits<double>::infinity();
        lastMassRefreshConverged_=false;
        std::array<double,3> prev{};
        bool havePrev=false;
        fdm_->SuspendIntegration();
        try {
            for (int iter=1; iter<=kMaxPasses; ++iter) {
                if (!fdm_->Run())
                    throw std::runtime_error("JSBSim zero-dt mass-property refresh failed.");
                const std::array<double,3> cur={
                    get("inertia/mass-slugs",std::numeric_limits<double>::quiet_NaN())*kSlugToKg,
                    get("inertia/cg-x-in",std::numeric_limits<double>::quiet_NaN())*0.0254,
                    get("inertia/iyy-slugs_ft2",std::numeric_limits<double>::quiet_NaN())*kSlugFt2ToKgM2
                };
                if (std::any_of(cur.begin(),cur.end(),[](double v){return !std::isfinite(v);}))
                    throw std::runtime_error("Non-finite derived mass property during zero-dt refresh.");
                lastMassRefreshIterations_=iter;
                if (havePrev) {
                    double e=0.0;
                    for (size_t i=0;i<cur.size();++i)
                        e=std::max(e,std::abs(cur[i]-prev[i])/std::max(1.0,std::abs(cur[i])));
                    lastMassRefreshError_=e;
                    if (iter>=kMinPasses && e<=kTol) {
                        lastMassRefreshConverged_=true;
                        break;
                    }
                }
                prev=cur;
            ...
            if (!lastMassRefreshConverged_)
                throw std::runtime_error("Derived mass properties did not converge during zero-dt refresh.");
        } catch (...) {
    ...
    void setLongitudinalWind(double windLongMps)
    {
        // Longitudinal-only validation: aircraft nominal heading is true north.
        // Positive is tailwind (air mass moving with the aircraft).
        set("atmosphere/wind-north-fps", windLongMps/kFtToM, false);
        set("atmosphere/wind-east-fps", 0.0, false);
        set("atmosphere/wind-down-fps", 0.0, false);
    }
    ...
    mxArray* diagnostics() const
    {
        const char* f[] = {"mass_kg","cg_x_m","Iyy_kgm2","alpha_rad","alphadot_radps","gamma_rad","theta_rad",
                           ...
                           "mass_refresh_iterations","mass_refresh_error","mass_refresh_converged"};
        mxArray* s=mxCreateStructMatrix(1,1,35,f);
        ...
        put("mass_refresh_iterations",static_cast<double>(lastMassRefreshIterations_));
        put("mass_refresh_error",lastMassRefreshError_);
        put("mass_refresh_converged",lastMassRefreshConverged_ ? 1.0 : 0.0);
        return s;
    }

    mxArray* continuousDiagnostics(double deltaN, int cfgId) const
    {
        mxArray* s = diagnostics();
        auto add=[&](const char* name,double value) {
            const int idx=mxAddField(s,name);
            if (idx < 0) throw std::runtime_error(std::string("Could not add diagnostic field: ")+name);
            mxSetFieldByNumber(s,0,idx,mxCreateDoubleScalar(value));
        };
        const double vn=get("velocities/v-north-fps",0.0)*kFtToM;
        const double ve=get("velocities/v-east-fps",0.0)*kFtToM;
        const double vd=get("velocities/v-down-fps",0.0)*kFtToM;
        add("continuous_time_s",continuousTimeS_);
        add("delta_n_m",deltaN);
        add("pos_n_m",continuousPosNM_);
        add("v_north_mps",vn);
        add("v_east_mps",ve);
        add("groundspeed_mps",std::hypot(vn,ve));
        add("vz_up_mps",-vd);
        add("wind_long_mps",get("atmosphere/wind-north-fps",continuousWindMps_/kFtToM)*kFtToM);
        add("wind_cross_mps",get("atmosphere/wind-east-fps",0.0)*kFtToM);
        add("cfg_id",static_cast<double>(cfgId));
        return s;
    }
    ...
    int lastMassRefreshIterations_=0;
    double lastMassRefreshError_=std::numeric_limits<double>::infinity();
    bool lastMassRefreshConverged_=false;
    bool continuousStarted_=false;
    int continuousCfg_=0;
    double continuousFuelScale_=1.0;
    double continuousWindMps_=0.0;
    double continuousPosNM_=0.0;
    double continuousTimeS_=0.0;
};
...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[])
{
    try {
        ...
        if (cmd == "start_continuous") {
            if (!gOracle) throw std::runtime_error("Oracle not initialized.");
            if (nrhs != 6) throw std::runtime_error("start_continuous(x,u,cfgId,fuelScale,windLongMps)");
            auto x=vectorN(prhs[1],kStateN,"x");
            auto u=vectorN(prhs[2],kInputN,"u");
            const double cfgRaw=scalar(prhs[3],"cfgId");
            if (std::abs(cfgRaw-std::round(cfgRaw))>1e-12) throw std::runtime_error("cfgId must be an integer.");
            ...
            const double wind=scalar(prhs[5],"windLongMps");
            mxArray* d=gOracle->startContinuous(x,u,cfg,fuel,wind);
            if (nlhs>0) plhs[0]=d; else mxDestroyArray(d);
            return;
        }
        if (cmd == "step_continuous") {
            if (!gOracle) throw std::runtime_error("Oracle not initialized.");
            if (nrhs != 6) throw std::runtime_error("step_continuous(u,cfgId,fuelScale,windLongMps,Ts)");
            auto u=vectorN(prhs[1],kInputN,"u");
            const double cfgRaw=scalar(prhs[2],"cfgId");
            if (std::abs(cfgRaw-std::round(cfgRaw))>1e-12) throw std::runtime_error("cfgId must be an integer.");
            const int cfg=static_cast<int>(std::llround(cfgRaw));
            const double fuel=scalar(prhs[3],"fuelScale");
            const double wind=scalar(prhs[4],"windLongMps");
            const double Ts=scalar(prhs[5],"Ts");
            auto out=gOracle->stepContinuous(u,cfg,fuel,wind,Ts);
            if (nlhs>0) plhs[0]=vectorMx(out.first);
            if (nlhs>1) plhs[1]=out.second; else mxDestroyArray(out.second);
            return;
        }
        ...
        if (cmd == "version") {
            if (nlhs>0) plhs[0]=mxCreateString("AirdropX JSBSim wind-continuous oracle v1.2.1+mass-refresh-v135");
            return;
        }
