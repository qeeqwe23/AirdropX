...
$stamp=Get-Date -Format 'yyyyMMdd_HHmmss'; $backup=Join-Path $ProjectRoot ("matlab\_backup_v133_"+$stamp)
...
CopySafe (Join-Path $Here 'run_wind_disturbance_airdrop_v133_D.ps1') (Join-Path $ProjectRoot 'run_wind_disturbance_airdrop_v133_D.ps1')
CopySafe (Join-Path $Here 'run_wind_disturbance_airdrop_point_v133_D.ps1') (Join-Path $ProjectRoot 'run_wind_disturbance_airdrop_point_v133_D.ps1')
Write-Host "Installed Physics-MPC v1.3.3 into $ProjectRoot"
...
Write-Host 'v1.3.3 adds event-gated recovery, bounded energy exchange, numerical input snapping, and hidden MATLAB child processes.'
Write-Host 'Run: .\run_wind_disturbance_airdrop_v133_D.ps1'
