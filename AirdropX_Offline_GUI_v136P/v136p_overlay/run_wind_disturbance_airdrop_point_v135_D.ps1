...
if(-not(Test-Path $cal)){throw 'Run the full v1.3.5 runner once to create wind disturbance calibration.'}
...
# v1.3.5 changes the C++ Oracle mass-transition refresh.  The installer deletes
# the old mex, but keep this runner self-healing for copied/manual installs.
...
if((-not(Test-Path $mex)) -or (-not(Test-Path $oracleMarker))){
  ...
  $p=StartMatlab $cmd $so $se; if($p.ExitCode -ne 0){if(Test-Path $so){Get-Content $so};if(Test-Path $se){Get-Content $se};throw 'v1.3.5 wind Oracle build failed.'}
}
if(-not(Test-Path $oracleMarker)){
  ...
  $p=StartMatlab $cmd $so $se; if($p.ExitCode -ne 0){if(Test-Path $so){Get-Content $so};if(Test-Path $se){Get-Content $se};throw 'v1.3.5 wind Oracle selftest failed; rebuild is required.'}
  Set-Content -Encoding ASCII -Path $oracleMarker -Value ("pass "+(Get-Date -Format o))
}

$out=Join-Path $ProjectRoot ("matlab\results\physics_mpc_v135_equivalent_energy_recovery_point\${Scenario}_${Mode}")
...
$cmd="cd('$root'); addpath('matlab'); addpath('matlab/phys_mpc'); addpath('matlab/wind'); addpath('matlab/airdrop'); r=airdropx_wind_airdrop_entry_v135(string(pwd),OutputRoot=string('$outEsc'),ScenarioName=string('$Scenario'),UseWindCompensation=$aware,UseWindDisturbanceMPC=$wm,UseFractionalRelease=$frac,UseWindConfidenceGate=$conf,UseUnifiedGustRecovery=$rec,WindCalibrationPath=string('$calEsc'),SensorNoiseSeed=$Seed); disp(r);"
