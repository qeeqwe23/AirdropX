...
function Invoke-MatlabCapture([string]$cmd,[string]$logPath){
  ...
  $p=Start-Process -FilePath $MatlabExe -ArgumentList "-batch `"$cmd`"" -RedirectStandardOutput $stdout -RedirectStandardError $stderr -Wait -PassThru
...
$out=Join-Path $ProjectRoot 'matlab\results\physics_mpc_v132_wind_recovery_airdrop_validation'; New-Item -ItemType Directory -Force -Path $out|Out-Null
...
$code=Invoke-MatlabCapture $cmd (Join-Path $out 'wind_oracle_selftest_terminal.txt'); if($code -ne 0){throw 'Continuous-wind Oracle self-test failed.'}
...
# Diagnostic only: preserve the v1.3.1 sampled-release A/B on calm.
...
$cmd="cd('$rootEsc'); addpath('matlab'); addpath('matlab/airdrop'); R=airdropx_wind_airdrop_finalize_v132(string('$outEsc'),string('$manifestEsc')); disp(R);"
$code=Invoke-MatlabCapture $cmd (Join-Path $out 'finalize_terminal.txt'); if($code -ne 0){throw 'v1.3.2 wind-recovery finalizer failed.'}
Get-Content (Join-Path $out 'wind_recovery_validation_summary.txt')
