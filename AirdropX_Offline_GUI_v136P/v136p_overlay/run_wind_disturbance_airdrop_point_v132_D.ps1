...
$root=E $ProjectRoot; $cal=Join-Path $ProjectRoot 'matlab\results\physics_mpc_v130_wind_disturbance_calibration\wind_disturbance_model_v130.mat'; if(-not(Test-Path $cal)){throw 'Run full v1.3.2 runner once to create wind disturbance calibration.'}
$out=Join-Path $ProjectRoot ("matlab\results\physics_mpc_v132_wind_recovery_point\${Scenario}_${Mode}"); New-Item -ItemType Directory -Force -Path $out|Out-Null
...
$cmd="cd('$root'); addpath('matlab'); addpath('matlab/phys_mpc'); addpath('matlab/wind'); addpath('matlab/airdrop'); r=airdropx_wind_airdrop_entry_v132(string(pwd),OutputRoot=string('$outEsc'),ScenarioName=string('$Scenario'),UseWindCompensation=$aware,UseWindDisturbanceMPC=$wm,UseFractionalRelease=$frac,UseWindConfidenceGate=$conf,UseUnifiedGustRecovery=$rec,WindCalibrationPath=string('$calEsc')); disp(r);"
$p=Start-Process -FilePath $MatlabExe -ArgumentList "-batch `"$cmd`"" -Wait -PassThru
exit $p.ExitCode
