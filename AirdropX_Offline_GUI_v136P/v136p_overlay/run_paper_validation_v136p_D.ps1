...
$out=Join-Path $ProjectRoot 'matlab\results\physics_mpc_v136p_paper_validation'; New-Item -ItemType Directory -Force -Path $out|Out-Null
...
$cmd="cd('$rootEsc'); addpath('matlab'); addpath('matlab/wind'); addpath('matlab/airdrop'); addpath('matlab/avionics'); p=airdropx_airdrop_ballistic_params_v121; assert(abs(p.calibration_error_m)<1e-8); T=airdropx_wind_airdrop_manifest_v136p(string('$manifestEsc')); disp(p); disp(T);"
...
$eqRoot=Join-Path $ProjectRoot 'matlab\results\physics_mpc_v136p_paper_base_equivalence_audit'; $eqEsc=E $eqRoot
$cmd="cd('$rootEsc'); addpath('matlab'); addpath('matlab/phys_mpc'); addpath('matlab/wind'); addpath('matlab/airdrop'); addpath('matlab/avionics'); R=airdropx_phys_mpc_base_equivalence_audit_v136p(string(pwd),OutputRoot=string('$eqEsc'),WindCalibrationPath=string('$calEsc'),SensorNoiseSeed=101); disp(R); assert(R.pass);"
$code=Invoke-MatlabCapture $cmd (Join-Path $out 'base_equivalence_terminal.txt'); if($code -ne 0){throw 'v1.3.6-Paper base-equivalence audit failed. Full suite intentionally not started.'}
$jobs=@()
...
function StartCase($j){
  ...
  $cmd="cd('$rootEsc'); addpath('matlab'); addpath('matlab/phys_mpc'); addpath('matlab/wind'); addpath('matlab/airdrop'); addpath('matlab/avionics'); r=airdropx_wind_airdrop_entry_v136p(string(pwd),OutputRoot=string('$dirEsc'),ScenarioName=string('$($j.Scenario)'),UseWindCompensation=$aware,UseWindDisturbanceMPC=$wm,UseFractionalRelease=$frac,UseWindConfidenceGate=$conf,UseUnifiedGustRecovery=$rec,UsePaperSensorModel=true,UseIndependentCargoTruth=false,WindCalibrationPath=string('$calEsc'),Duration_s=$($j.Duration),TargetStart_m=$($j.TargetStart),TargetSpacing_m=$($j.Spacing),SensorNoiseSeed=$($j.Seed)); disp(r);"
...
$cmd="cd('$rootEsc'); addpath('matlab'); addpath('matlab/airdrop'); R=airdropx_wind_airdrop_finalize_v136p(string('$outEsc'),string('$manifestEsc')); disp(R);"
$code=Invoke-MatlabCapture $cmd (Join-Path $out 'finalize_terminal.txt'); if($code -ne 0){throw 'v1.3.6-Paper transient-energy finalizer failed.'}
Get-Content (Join-Path $out 'paper_validation_summary.txt')
