...
class MainWindow(QMainWindow):
    def __init__(self, app_root: Path):
        ...
        self.current_backend_mode = "v136p"
        self.setWindowTitle("AirdropX MQ-9 Reaper Cargo 空投仿真与测控平台 v2.3 · v1.3.6-Paper 默认")
        ...
        outer.setSpacing(3)
        top = QHBoxLayout()
        title = QLabel("AirdropX MQ-9 Reaper Cargo 空投仿真与测控平台 v2.3")
        title.setObjectName("Title")
        self.state = QLabel("任务状态: 待机  后端: Physics-MPC v1.3.6-Paper")
        ...
        self.statusBar().showMessage(
            f"Python {platform.python_version()} | PyQt6 | 离线模式 | 默认后端 v1.3.6-Paper"
        )
        ...
        self.config.backend_changed.connect(self._backend_changed)
        self._backend_changed("v136p")

    def _backend_changed(self, mode: str):
        self.current_backend_mode = mode
        self.monitor.set_backend(mode)
        label = "Physics-MPC v1.3.6-Paper" if mode == "v136p" else "Physics-MPC v1.4.0 (实验)"
        if not (self.worker and self.worker.isRunning()):
            self.state.setText(f"任务状态: 待机  后端: {label}")
        self.statusBar().showMessage(
            f"Python {platform.python_version()} | PyQt6 | 离线模式 | 当前后端 {label}"
        )
    ...
    def start(self, cfg):
        ...
        self.state.setText(f"任务状态: 运行中  后端: {cfg.backend_label}")
        self.progress.setValue(1)
        self.analysis.append_log(f"[GUI] 启动 {cfg.backend_label} 后端...")
    ...
    def reset(self):
        ...
        self.analysis.reset()
        self.progress.setValue(0)
        label = "Physics-MPC v1.3.6-Paper" if self.current_backend_mode == "v136p" else "Physics-MPC v1.4.0 (实验)"
        self.state.setText(f"任务状态: 待机  后端: {label}")
    ...
    def _done(self, root):
        try:
            ...
            self.state.setText(f"任务状态: 完成  后端: {self.current_cfg.backend_label}")
