...
[bankOK,bankHas]=localBankHas(baseBank,H,V,fuelScale); windOK=localWindMatches(baseWind,H,V,fuelScale);
if bankOK && bankHas && windOK, M=struct("bankPath",baseBank,"windPath",baseWind,"source","existing_exact_model","built",false); return; end
if policy=="certified_only", error("AirdropX:GUI:NoExistingPoint","No complete existing H=%.6g V=%.6g fuel=%.4g model+wind pair exists.",H,V,fuelScale); end
...
if bankOK && bankHas, selectedBank=baseBank; else
    selectedBank=cacheBank; [~,cacheHas]=localBankHas(cacheBank,H,V,fuelScale);
    if ~cacheHas
        if exist("airdropx_phys_build_bank","file")~=2, error("AirdropX:GUI:MissingBankBuilder","airdropx_phys_build_bank.m is required for a new H/V point."); end
        fprintf("[GUI_MODEL] building exact v1.4.0 physics vertices H=%.6g V=%.6g\n",H,V);
        ...
        [~,cacheHas]=localBankHas(cacheBank,H,V,fuelScale); if ~cacheHas, error("AirdropX:GUI:BankBuildFailed","Generated bank does not contain all five exact cfg vertices."); end
    end
end
if ~localWindMatches(cacheWind,H,V,fuelScale)
...
try, S=load(path,"rows"); r=S.rows; m=abs(r.H_m-H)<=1e-9 & abs(r.V_mps-V)<=1e-9 & abs(r.fuel_scale-fuel)<=1e-12 & ismember(r.cfg,0:4) & logical(r.pass); has=sum(m)==5 && numel(unique(r.cfg(m)))==5; catch, has=false; end
end
function tf=localWindMatches(path,H,V,fuel)
tf=false; if ~isfile(path), return; end
try, S=load(path,"W"); W=S.W; tf=isfield(W,"pass") && W.pass && abs(W.H-H)<=1e-9 && abs(W.V-V)<=1e-9 && abs(W.FuelScale-fuel)<=1e-12; catch, tf=false; end
end
