function M=airdropx_gui_prepare_model_v136p(projectRoot,H,V,fuelScale,policy)
%AIRDROPX_GUI_PREPARE_MODEL_V136P Resolve/build an exact H/V physics+wind model pair.
arguments
...
if bankOK && bankHas && windOK
    M=struct("bankPath",baseBank,"windPath",baseWind,"source","existing_exact_model","built",false); return
end
if policy=="certified_only"
    error("AirdropX:GUI:NoExistingPoint", ...
        "No complete existing H=%.6g V=%.6g fuel=%.4g model+wind pair exists. Switch H/V policy to auto-cache or build the point first.",H,V,fuelScale);
end
cacheRoot=fullfile(projectRoot,"matlab","results","offline_gui_v136p_model_cache",sprintf("H%07.2f_V%06.2f_F%04.2f",H,V,fuelScale));
if ~isfolder(cacheRoot), mkdir(cacheRoot); end
cacheBank=fullfile(cacheRoot,"physics_bank.mat");
cacheWind=fullfile(cacheRoot,"wind_disturbance_model_v130.mat");
if bankOK && bankHas
    ...
    if ~cacheHas
        ...
        fprintf("[GUI_MODEL] building exact v1.3.6-compatible physics vertices H=%.6g V=%.6g\n",H,V);
        ...
        [~,cacheHas]=localBankHas(cacheBank,H,V,fuelScale);
        if ~cacheHas
            error("AirdropX:GUI:BankBuildFailed","Generated bank does not contain all five exact cfg vertices.");
        end
    end
end
cacheWindOK=localWindMatches(cacheWind,H,V,fuelScale);
if ~cacheWindOK
    fprintf("[GUI_MODEL] calibrating v1.3.6 longitudinal disturbance map H=%.6g V=%.6g\n",H,V);
...
try
    S=load(path,"W"); W=S.W;
    tf=isfield(W,"pass") && W.pass && abs(W.H-H)<=1e-9 && abs(W.V-V)<=1e-9 && abs(W.FuelScale-fuel)<=1e-12;
catch
    tf=false;
end
end
