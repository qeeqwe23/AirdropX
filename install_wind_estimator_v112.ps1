...
$stamp=Get-Date -Format 'yyyyMMdd_HHmmss'; $backup=Join-Path $matlab ("wind_pre_v112_"+$stamp); New-Item -ItemType Directory -Force -Path $backup | Out-Null
...
Copy-Item -Force (Join-Path $here 'run_wind_estimator_validation_v112_D.ps1') (Join-Path $ProjectRoot 'run_wind_estimator_validation_v112_D.ps1')
Copy-Item -Force (Join-Path $here 'run_wind_estimator_point_v112_D.ps1') (Join-Path $ProjectRoot 'run_wind_estimator_point_v112_D.ps1')
Write-Host "Installed AirdropX v1.1.2 longitudinal wind estimator into $dst"
Write-Host "No MEX/C++/Physics-MPC files were replaced."
Write-Host "Run: .\run_wind_estimator_validation_v112_D.ps1"
