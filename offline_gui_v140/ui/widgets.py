...
class LineChart(QWidget):
    ...
    def paintEvent(self, _event):
        ...
        def mappt(xv, yv):
            return QPointF(r.left() + (xv - xmin) / (xmax - xmin) * r.width(), r.bottom() - (yv - ymin) / (ymax - ymin) * r.height())

        for x, y, c, label, style in self.lines:
            pts = [mappt(a, b) for a, b in zip(x, y) if isfinite(a) and isfinite(b)]
            if len(pts) >= 2:
                p.setPen(QPen(c, 1.5, style))
                p.drawPolyline(QPolygonF(pts))
        for xv, c, label in self.markers:
            if xmin <= xv <= xmax:
...
class ScatterChart(QWidget):
    ...
    def paintEvent(self, _event):
        ...
        span = max([abs(x) for x in self.points] + [20.0]) * 1.2
        cx = r.center().x()
        cy = r.center().y()
...
class AttitudeWidget(QWidget):
    ...
    def paintEvent(self, _event):
        ...
        p.drawText(4, 14, f"Pitch: {self.pitch:+.1f} deg")
