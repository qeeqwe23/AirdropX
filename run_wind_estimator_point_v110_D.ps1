...
$out=Join-Path $ProjectRoot ("matlab\results\physics_mpc_v110_wind_point\"+$Scenario); New-Item -ItemType Directory -Force -Path $out | Out-Null
$cmd="cd('$(E $ProjectRoot)'); addpath('matlab'); addpath('matlab/wind'); addpath('matlab/sfunc_jsbsim'); r=airdropx_wind_estimation_entry_v110(string(pwd),OutputRoot=string('$(E $out)'),ScenarioName=string('$Scenario'),Duration_s=$dur); disp(r);"
& $MatlabExe -batch $cmd
