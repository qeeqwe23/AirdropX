...
$stamp=Get-Date -Format 'yyyyMMdd_HHmmss'; $backup=Join-Path $ProjectRoot ("matlab\phys_mpc_controller_pre_v081_"+$stamp); New-Item -ItemType Directory -Force -Path $backup | Out-Null
...
Copy-Item -Force (Join-Path $here 'run_phys_mpc_preview_envelope_diagnostic_D.ps1') (Join-Path $ProjectRoot 'run_phys_mpc_preview_envelope_diagnostic_D.ps1')
Write-Host "Installed Physics-MPC v0.8.1 diagnostic full-flow overlay into $ProjectRoot"
Write-Host 'This version DOES NOT relax Richardson/Q/R/mission gates.'
...
Write-Host 'Run: .\run_phys_mpc_preview_envelope_diagnostic_D.ps1'
