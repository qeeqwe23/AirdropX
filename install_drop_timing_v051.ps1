...
$backup=Join-Path $ProjectRoot ("matlab\phys_mpc_controller_pre_v051_"+$stamp)
...
Copy-Item -Force (Join-Path $here 'run_phys_mpc_drop_timing_compare_D.ps1') (Join-Path $ProjectRoot 'run_phys_mpc_drop_timing_compare_D.ps1')
Write-Host "Installed Physics-MPC v0.5.1 drop-timing comparison overlay into $ProjectRoot"
Write-Host "No Oracle MEX, S-Function MEX, C++ source, physics_bank.mat, Q/R, horizon, bounds, or certification result was modified."
Write-Host "Run: .\run_phys_mpc_drop_timing_compare_D.ps1 -Mode Compare"
