...
class WindConfig:
    ...
    def along_track_mps(self) -> float:
        ...
        wind convention used by the v1.3.6-Paper AirdropX mission.
        """
...
@dataclass
class MissionConfig:
    # Kept in JSON for bridge compatibility; software v1.0 is fixed to v1.3.6-Paper.
    backend_mode: str = "v136p"
    ...
    def validate(self) -> list[str]:
        errors: list[str] = []
        if self.backend_mode != "v136p":
            errors.append("软件版控制器固定为 Physics-MPC v1.3.6-Paper。")
    ...
    def backend_label(self) -> str:
        return "Physics-MPC v1.3.6-Paper"

    def to_dict(self) -> dict:
        d = asdict(self)
        d["backend_mode"] = "v136p"
