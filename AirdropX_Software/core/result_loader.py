...
class MissionResult:
    ...
    def bool_from_summary(self, *keys: str) -> bool | None:
        for key in keys:
            value = self.metric_from_summary(key)
            if value is None:
                continue
...
def load_result(output_root: str | Path) -> MissionResult:
    ...
    meta_path = root / "gui_backend_meta.json"
    ...
    if meta_path.exists():
        try:
            ...
            meta = {}
    return MissionResult(ts, cargo, summary, root, meta)