...
def find_project_root(app_root: Path, explicit: str = "") -> Path:
    ...
    env = os.environ.get("AIRDROPX_PROJECT_ROOT")
    if env:
        p = Path(env).resolve()
...
class MatlabBackend:
    """Thin GUI bridge to the frozen v1.3.6-Paper controller."""
    ...
    def run(
        ...
        output_root = Path(cfg.output_root) if cfg.output_root else (
            paths.project_root / "matlab" / "results" / "AirdropX_Offline" / stamp
        )
        ...
        mcmd = (
            ...
            f"airdropx_gui_backend_entry_v136p('{matlab_quote(paths.project_root)}','{matlab_quote(config_path)}');"
        )
        cmd = [str(paths.matlab_exe), "-batch", mcmd]
        if log_cb:
            log_cb(f"[GUI] 输出目录: {output_root}")
            log_cb(f"[GUI] MATLAB: {paths.matlab_exe}")
            log_cb("[GUI] 控制器: Physics-MPC v1.3.6-Paper")
            ...
            if cfg.wind.mode == "formal":
                log_cb("[GUI] 论文预设使用 v1.3.6-Paper 对应场景的固定传感器噪声种子。")
