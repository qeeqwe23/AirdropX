...
$out=Join-Path $ProjectRoot 'matlab\results\physics_mpc_v140_sensor_base_equivalence_audit'; New-Item -ItemType Directory -Force -Path $out|Out-Null
...
$cmd="cd('$root'); addpath('matlab'); addpath('matlab/phys_mpc'); addpath('matlab/wind'); addpath('matlab/airdrop'); addpath('matlab/avionics'); R=airdropx_phys_mpc_base_equivalence_audit_v140(string(pwd),OutputRoot=string('$(E $out)'),WindCalibrationPath=string('$(E $cal)'),SensorNoiseSeed=$Seed); disp(R); if ~R.pass, error('AirdropX:WindMPC:BaseEquivalenceFailed','Base equivalence audit failed.'); end"
