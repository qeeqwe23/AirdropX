function report=airdropx_phys_build_speed_slice_v070(projectRoot,opts)
%AIRDROPX_PHYS_BUILD_SPEED_SLICE_V070 Build one non-V50 pilot speed slice without closing Oracle in-process.
% Formal pilot slice: H=[200 110 20] m, cfg0..4, fuel=1, Ts=0.1 s.
% Seeds come from the already validated V=50 bank at the same H/cfg.
arguments
...
if ~any(abs(opts.Speed_mps-allowed)<=1e-12)
    error("AirdropX:PhysMPC:SpeedPilotGrid","Speed slice must be one of [40 45 55 60] m/s; V50 is reused from v0.3.3 bank.");
end
...
localStatus(status,"STARTED",sprintf("V=%.1f H=[200 110 20] cfg=0:4",opts.Speed_mps));
try
    ...
    if ~isfield(info,"version") || ~contains(string(info.version),"v0.3.3")
        error("AirdropX:PhysMPC:OldOracle","Speed pilot requires Physics Oracle v0.3.3.");
    end
    localStatus(status,"ORACLE_INIT_DONE",string(info.version));
    % Structural mass/fuel/configuration audit is kept from v0.3.3 build semantics.
    ...
    heights=[200 110 20]; cfgs=0:4; n=numel(heights)*numel(cfgs);
    ...
    report=struct("version","Physics-MPC v0.7.0 speed-slice builder","pass",all(rows.pass),"speed_mps",opts.Speed_mps,"rows",rows,"vertex_count",height(rows));
