function build_airdropx_jsbsim_wind_oracle_v121(jsbsimRoot)
%BUILD_AIRDROPX_JSBSIM_WIND_ORACLE_V121 Transactional build of separate wind-continuous Oracle.
...
% than the API used by v0.3.3, instead of surfacing a long compiler error later.
...
src=fullfile(thisDir,"airdropx_jsbsim_wind_oracle_mex.cpp");
...
base="airdropx_jsbsim_wind_oracle_mex";
try
    ...
    fprintf(2,"Wind Oracle candidate build FAILED; existing wind-oracle MEX was not touched.\n");
    rethrow(ME);
end
...
clear airdropx_jsbsim_wind_oracle_mex;
try
    v=feval(base,"version");
    assert(contains(string(v),"v1.2.1"),"Candidate Oracle returned unexpected version: %s",string(v));
catch ME
    clear airdropx_jsbsim_wind_oracle_mex;
    fprintf(2,"Wind Oracle candidate load/version check FAILED; existing wind-oracle MEX was not touched.\n");
    rethrow(ME);
end
clear airdropx_jsbsim_wind_oracle_mex;
cd(oldPwd);
...
try
    ...
    assert(contains(string(vInstalled),"v1.2.1"), ...
    ...
    if hadTarget && strlength(backup)>0 && isfile(backup)
        ...
        rehash;
        fprintf(2,"Wind Oracle install/load validation FAILED; previous working MEX was restored from %s.\n",backup);
    elseif ~hadTarget && isfile(target)
        delete(target);
        rehash;
        fprintf(2,"Wind Oracle install/load validation FAILED; incomplete new target was removed.\n");
    end
    rethrow(ME);
end
...
if ~isempty(missing)
    error("AirdropX:PhysMPC:JSBSimApiMismatch", ...
        "JSBSim header %s lacks required v0.3.3 API symbol(s): %s", ...
