function report=airdropx_phys_finalize_envelope_v082(projectRoot,opts)
%AIRDROPX_PHYS_FINALIZE_ENVELOPE_V082 Aggregate 95 diagnostic PreviewOnly HxV cases.
...
report=struct("version","Physics-MPC v0.8.2 fixed-horizon diagnostic full-flow HxV envelope","pass",formalPass,"formal_envelope_pass",formalPass,"physics_certification_pass",logical(bankAudit.certification_pass), ...
...
fprintf(fid,"Physics-MPC v0.8.2 fixed-horizon diagnostic continue-on-certification-failure HxV envelope\n");
...
fprintf(fid,"common_controller_pass=%d\ncommon_Q_max_diff=%.9g\ncommon_R_max_diff=%.9g\nmpc_horizon_fixed=%d\nhorizon_same=%d\nNp=%d\nNc=%d\nterminal_horizon_same=%d\nNp_required_range=[%.9g %.9g]\nrho_cl_range=[%.9g %.9g]\ndA_rel_max=%.9g\ndB_rel_max=%.9g\n",a.common_controller_pass,a.max_Q_diff,a.max_R_diff,a.mpc_horizon_fixed,a.same_horizon,a.Np,a.Nc,a.terminal_horizon_same,a.Np_required_min,a.Np_required_max,a.rho_min,a.rho_max,a.dA_rel_max,a.dB_rel_max);
...
try
    ...
    fig=figure('Visible','off','Color','w','Position',[100 100 1300 900]); tl=tiledlayout(fig,2,2,'TileSpacing','compact','Padding','compact'); localHeat(nexttile(tl),V,H,Zq,'peak |q| (deg/s)'); localHeat(nexttile(tl),V,H,Zn,'peak normalized'); localHeat(nexttile(tl),V,H,Zh,'peak |h| (m)'); localHeat(nexttile(tl),V,H,Zr,'recovery after last drop (s)'); title(tl,'Physics-MPC v0.8.2 fixed-horizon diagnostic full HxV flow','Interpreter','none'); exportgraphics(fig,fullfile(out,'envelope_validation.png'),'Resolution',160); close(fig);
catch
end
end
