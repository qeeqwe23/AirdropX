function report=airdropx_phys_merge_envelope_bank_v081(projectRoot,opts)
%AIRDROPX_PHYS_MERGE_ENVELOPE_BANK_V081 Merge all 475 attempted vertices for diagnostic full-flow use.
...
% Common-controller audit over every computationally usable vertex. Missing
% vertices are not synthesized or replaced by a neighbor.
base=[]; maxQ=0; maxR=0; maxSS=0; maxUS=0; sameHorizon=true; rhoMin=inf; rhoMax=-inf; dAMax=0; dBMax=0; usablePayloadCount=0;
for k=1:expected
    if ~rows.usable(k), continue; end
    v=vertices{k};
    if isempty(v), rows.usable(k)=false; continue; end
    q=double(v.Q); rr=double(v.R); ss=double(v.qrMeta.StateScale(:)); us=double(v.qrMeta.InputScale(:)); np=double(v.terminal.Np); nc=double(v.terminal.Nc);
...
commonUnified=maxQ<=1e-12 && maxR<=1e-12 && maxSS<=1e-12 && maxUS<=1e-12 && sameHorizon;
certificationPass=all(rows.pass) && commonUnified; diagnosticRunnable=all(rows.usable) && usablePayloadCount==expected && commonUnified;
bankAudit=struct("pass",certificationPass,"certification_pass",certificationPass,"diagnostic_runnable",diagnosticRunnable, ...
    ...
    "max_Q_diff",maxQ,"max_R_diff",maxR,"max_state_scale_diff",maxSS,"max_input_scale_diff",maxUS,"same_horizon",sameHorizon, ...
    "Np",base.Np,"Nc",base.Nc,"rho_min",rhoMin,"rho_max",rhoMax,"dA_rel_max",dAMax,"dB_rel_max",dBMax);
...
report=struct("version","Physics-MPC v0.8.1 diagnostic full HxV envelope bank","pass",certificationPass,"diagnostic_runnable",diagnosticRunnable, ...
...
a=r.bank_audit;
fprintf(fid,"Physics-MPC v0.8.1 diagnostic full HxV envelope bank\n");
...
fprintf(fid,"common_controller_pass=%d\ncommon_Q_max_diff=%.9g\ncommon_R_max_diff=%.9g\ncommon_state_scale_max_diff=%.9g\ncommon_input_scale_max_diff=%.9g\nhorizon_same=%d\nNp=%d\nNc=%d\n", ...
    a.common_controller_pass,a.max_Q_diff,a.max_R_diff,a.max_state_scale_diff,a.max_input_scale_diff,a.same_horizon,a.Np,a.Nc);
