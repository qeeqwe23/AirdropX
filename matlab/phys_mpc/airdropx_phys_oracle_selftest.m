...
requiredFinite=[d.mass_kg d.cg_x_m d.Iyy_kgm2 d.qbar_Pa d.alphadot_radps ...
    d.udot_mps2 d.wdot_mps2 d.qdot_radps2 d.algebraic_settle_error ...
    d.elevator_cmd_norm d.elevator_pos_norm d.throttle_cmd_norm d.throttle_pos_norm];
controlError=[d.elevator_cmd_norm-u(1); d.elevator_pos_norm-u(1); ...
              d.throttle_cmd_norm-u(2); d.throttle_pos_norm-u(2)];
controlOk=all(isfinite(controlError)) && norm(controlError,inf)<=1e-9;
algebraicOk=isfield(d,"algebraic_settle_converged") && logical(d.algebraic_settle_converged);
physicalOk=all(isfinite(requiredFinite)) && ...
    d.mass_kg>100 && d.Iyy_kgm2>10 && d.qbar_Pa>0 && fuelFrozen && controlOk && algebraicOk;
...
report=struct("states",X,"spread",spread,"max_abs_state_spread",maxSpread, ...
    "physical_ok",physicalOk,"fuel_frozen",fuelFrozen,"control_ok",controlOk,"control_error",controlError, ...
    "algebraic_ok",algebraicOk,"algebraic_iterations",d.algebraic_settle_iterations,"algebraic_error",d.algebraic_settle_error, ...
...
if ~report.pass && opts.ThrowOnFail
    error("AirdropX:PhysMPC:OracleSelfTestFailed", ...
        ...
        deterministic,maxSpread,pathIndependent,norm(pathError,inf), ...
        pathError(1),pathError(2),pathError(3),pathError(4),pathError(5),pathError(6),pathError(7), ...
        physicalOk,fuelFrozen,controlOk,algebraicOk,d.algebraic_settle_iterations,d.algebraic_settle_error,semigroup.pass,semigroup.max_scaled);
end
end
