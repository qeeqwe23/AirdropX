function report=airdropx_phys_preview_speed_entry_v070(projectRoot,opts)
%AIRDROPX_PHYS_PREVIEW_SPEED_ENTRY_V070 One speed-pilot PreviewOnly mission per MATLAB process.
arguments
...
validH=[20 110 200]; validV=[40 45 50 55 60];
if ~any(abs(validH-opts.Height_m)<=1e-12), error("AirdropX:PhysMPC:SpeedPilotHeight","Pilot H must be one of [20 110 200] m."); end
if ~any(abs(validV-opts.Speed_mps)<=1e-12), error("AirdropX:PhysMPC:SpeedPilotGrid","Pilot V must be one of [40 45 50 55 60] m/s."); end
if ~isfile(opts.BankPath), error("AirdropX:PhysMPC:BankMissing","Speed pilot bank missing: %s",opts.BankPath); end
...
localStatus(status,"STARTED",sprintf("H=%.1f V=%.1f PreviewOnly",opts.Height_m,opts.Speed_mps));
try
