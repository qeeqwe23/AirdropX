function report=airdropx_phys_runtime_command_closed_loop_v100(projectRoot,opts)
%AIRDROPX_PHYS_RUNTIME_COMMAND_CLOSED_LOOP_V100 One uninterrupted moving H/V command nonlinear mission.
arguments
...
if any(abs(opts.DropTimes_s-[60 60.2 60.4 60.6])>1e-12), error("AirdropX:PhysMPC:RuntimeDropSchedule","v1.0 validation fixes close-spaced releases at [60 60.2 60.4 60.6] s."); end
if opts.Duration_s<140, error("AirdropX:PhysMPC:RuntimeTooShort","v1.0 runtime validation needs >=140 s."); end
...
try
    ...
    bank=airdropx_phys_runtime_prepare_bank_v100(opts.MasterBankPath); Ts=bank.Ts; N=bank.N; Nsim=ceil(opts.Duration_s/Ts); t=(0:Nsim-1)'*Ts;
    [Hcmd,Vcmd,profileMeta]=airdropx_phys_runtime_command_profile_v100(opts.ScenarioName,t);
    if numel(Hcmd)~=Nsim || numel(Vcmd)~=Nsim, error("AirdropX:PhysMPC:RuntimeProfileSize","Profile size mismatch."); end
    initStage=airdropx_phys_runtime_interpolate_stage_v100(bank,Hcmd(1),Vcmd(1),0,NeedTerminal=false); x=initStage.xref;
    ...
    trajectoryOracleSelftest=airdropx_phys_oracle_selftest(x,initStage.uref,bank.pByCfg{1},ThrowOnFail=true);
    ...
    BuildTime=zeros(Nsim,1); QPTime=zeros(Nsim,1); TotalCompute=zeros(Nsim,1); PredErrInf=zeros(Nsim,1); Mass=nan(Nsim,1); Cg=nan(Nsim,1); Iyy=nan(Nsim,1); SourceCertMin=zeros(Nsim,1);
    warmAbs=[]; warmSlack=[]; prevCfg=0;
    for k=1:Nsim
        now=t(k); cfgNow=min(4,sum(now+1e-10>=opts.DropTimes_s));
        if cfgNow~=prevCfg, DropEvent(k)=true; fprintf("[DROP] t=%.2f cfg%d->cfg%d\n",now,prevCfg,cfgNow); prevCfg=cfgNow; end
        currentStage=airdropx_phys_runtime_interpolate_stage_v100(bank,Hcmd(k),Vcmd(k),cfgNow,NeedTerminal=false); dx=airdropx_phys_mpc_state_error(x,currentStage.xref);
        X(k,:)=x.'; RefX(k,:)=currentStage.xref.'; Err(k,:)=dx.'; RefU(k,:)=currentStage.uref.'; Cfg(k)=cfgNow;
        ft=now+(0:N)*Ts;
        if mode=="known_reference"
            [Hseq,Vseq]=airdropx_phys_runtime_command_profile_v100(opts.ScenarioName,ft);
        else
            Hseq=repmat(Hcmd(k),1,N+1); Vseq=repmat(Vcmd(k),1,N+1);
        end
        cfgSeq=sum(ft(:)>=opts.DropTimes_s,2)'; cfgSeq=min(4,cfgSeq);
        tb=tic; ctrl=airdropx_phys_mpc_runtime_condense_v100(bank,Hseq,Vseq,cfgSeq); BuildTime(k)=toc(tb); SourceCertMin(k)=ctrl.source_certified_count_min;
        sol=airdropx_phys_mpc_preview_solve(ctrl,x,warmAbs,warmSlack); QPTime(k)=sol.solve_time_s; TotalCompute(k)=BuildTime(k)+QPTime(k); Exit(k)=sol.exitflag;
        ...
        x=xNext; [warmAbs,warmSlack]=airdropx_phys_mpc_preview_shift_warmstart(sol);
        if k==1 || DropEvent(k) || mod(k,100)==0 || k==Nsim
            fprintf("[V100] %4d/%4d t=%6.1f Hcmd=%7.2f Vcmd=%5.2f cfg=%d hErr=% .3f VaErr=% .3f q=% .3fdeg/s build=%.2fms qp=%.2fms total=%.2fms pred=%.3g\n", ...
                k,Nsim,now,Hcmd(k),Vcmd(k),cfgNow,dx(1),dx(2),rad2deg(dx(5)),1e3*BuildTime(k),1e3*QPTime(k),1e3*TotalCompute(k),PredErrInf(k));
        end
    end
    finalStage=airdropx_phys_runtime_interpolate_stage_v100(bank,Hcmd(end),Vcmd(end),4,NeedTerminal=false); dxFinal=airdropx_phys_mpc_state_error(x,finalStage.xref);
    primaryNorm=max(abs(Err(:,1:5))./bank.stateScale(1:5).',[],2); tailStart=max(1,Nsim-round(5/Ts)+1);
    finalNorm=max(abs(dxFinal)./bank.stateScale); tailRms=max(sqrt(mean((Err(tailStart:end,:)./bank.stateScale.').^2,1)));
    ...
    hdot=gradient(Hcmd,Ts); vdot=gradient(Vcmd,Ts);
    metrics=struct("peak_h_err_m",max(abs(Err(:,1))),"peak_Va_err_mps",max(abs(Err(:,2))),"peak_gamma_err_deg",max(abs(rad2deg(Err(:,3)))),"peak_theta_err_deg",max(abs(rad2deg(Err(:,4)))),"peak_q_err_dps",max(abs(rad2deg(Err(:,5)))), ...
        ...
        "H_cmd_rate_max_mps",max(abs(hdot)),"V_cmd_rate_max_mps2",max(abs(vdot)),"source_certified_corner_count_min",min(SourceCertMin),"controller_restart_count",0,"retrim_count",0,"reid_count",0);
    ...
    gate.peak_primary=metrics.peak_primary_normalized<=opts.MaxPeakPrimaryNormalized; gate.final_normalized=finalNorm<=opts.MaxFinalNormalizedInf; gate.tail5s=tailRms<=opts.MaxTail5sNormalizedRms; gate.realtime_solver=metrics.qp_time_p95_ms<=1000*Ts*opts.MaxComputeP95FractionOfTs; gate.realtime_total=metrics.total_compute_p95_ms<=1000*Ts*opts.MaxComputeP95FractionOfTs;
    gate.no_restart=metrics.controller_restart_count==0 && metrics.retrim_count==0 && metrics.reid_count==0; vals=struct2cell(gate); gate.pass=all(cellfun(@(z)logical(z),vals));
    T=table(t,Hcmd,Vcmd,Cfg,X(:,1),RefX(:,1),Err(:,1),X(:,2),RefX(:,2),Err(:,2),rad2deg(X(:,3)),rad2deg(Err(:,3)),rad2deg(X(:,4)),rad2deg(Err(:,4)),rad2deg(X(:,5)),rad2deg(Err(:,5)),U(:,1),U(:,2),RefU(:,1),RefU(:,2),Mass,Cg,Iyy,Exit,1e3*BuildTime,1e3*QPTime,1e3*TotalCompute,PredErrInf,SourceCertMin, ...
        'VariableNames',{'t_s','H_cmd_m','Va_cmd_mps','cfg','h_m','h_ref_m','h_err_m','Va_mps','Va_ref_mps','Va_err_mps','gamma_deg','gamma_err_deg','theta_deg','theta_err_deg','q_dps','q_err_dps','elevator_cmd','throttle_cmd','elevator_trim','throttle_trim','mass_kg','cg_x_m','Iyy_kgm2','qp_exitflag','model_build_ms','qp_solve_ms','total_compute_ms','prediction_error_norm_inf','source_certified_corner_count_min'});
    ...
    report=struct("version","Physics-MPC v1.0.0 runtime H/V command mission","pass",gate.pass,"scenario",opts.ScenarioName,"preview_mode",mode,"options",opts,"profile_meta",profileMeta,"bank_audit",bank.audit,"oracle_version",ov,"oracle_selftest",trajectoryOracleSelftest,"metrics",metrics,"gate",gate,"completed_at",datetime("now"));
    save(fullfile(opts.OutputRoot,'runtime_command_mission.mat'),'report','T','-v7.3'); localWrite(report,fullfile(opts.OutputRoot,'runtime_command_summary.txt'));
    if ~report.pass && opts.ThrowOnFail, error("AirdropX:PhysMPC:RuntimeMissionFailed","Runtime command mission failed gates."); end
catch ME
...
m=r.metrics; fprintf(fid,'Physics-MPC v1.0.0 runtime continuous H/V command mission\nscenario=%s\npreview_mode=%s\npass=%d\n',r.scenario,r.preview_mode,r.pass); fprintf(fid,'drop_schedule_s=60 60.2 60.4 60.6\nNp=100\nNc=100\nq_soft=0\n');
...
fprintf(fid,'peak_h_err_m=%.9g\npeak_Va_err_mps=%.9g\npeak_q_err_dps=%.9g\npeak_primary_normalized=%.9g\nfinal_normalized_inf=%.9g\ntail5s_normalized_rms=%.9g\n',m.peak_h_err_m,m.peak_Va_err_mps,m.peak_q_err_dps,m.peak_primary_normalized,m.final_normalized_inf,m.tail5s_normalized_rms);
...
try
    f=figure('Visible','off','Position',[100 100 1500 1000]); tl=tiledlayout(f,3,2,'TileSpacing','compact','Padding','compact'); title(tl,'Physics-MPC v1.0 runtime commands - '+name);
    ...
    nexttile; plot(T.t_s,T.q_err_dps); ylabel('q err (deg/s)'); xlabel('t (s)'); grid on;
    ...
    nexttile; semilogy(T.t_s,max(T.prediction_error_norm_inf,1e-8)); ylabel('1-step pred norm'); xlabel('t (s)'); grid on;
    for ax=findall(f,'Type','axes')', for d=dropTimes, xline(ax,d,':'); end, end
    exportgraphics(f,fullfile(out,'runtime_command_tracking.png'),'Resolution',160); close(f);
