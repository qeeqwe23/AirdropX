...
requiredFinite=[d.mass_kg d.cg_x_m d.Iyy_kgm2 d.qbar_Pa ...
    d.udot_mps2 d.wdot_mps2 d.qdot_radps2 ...
...
physicalOk=all(isfinite(requiredFinite)) && ...
    d.mass_kg>100 && d.Iyy_kgm2>10 && d.qbar_Pa>0 && fuelFrozen && controlOk;
...
report=struct("states",X,"spread",spread,"max_abs_state_spread",maxSpread, ...
    "physical_ok",physicalOk,"fuel_frozen",fuelFrozen,"control_ok",controlOk,"control_error",controlError,"deterministic",deterministic, ...
...
if ~report.pass && opts.ThrowOnFail
    error("AirdropX:PhysMPC:OracleSelfTestFailed", ...
        ["Oracle self-test failed: deterministic=%d maxSpread=%.3g " + ...
         ...
         "physicalOk=%d fuelFrozen=%d controlOk=%d semigroupPass=%d semigroupMaxScaled=%.3g"], ...
        ...
        physicalOk,fuelFrozen,controlOk,semigroup.pass,semigroup.max_scaled);
end
end
