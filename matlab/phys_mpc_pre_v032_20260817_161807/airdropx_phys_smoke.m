...
if opts.OutputRoot==""
    opts.OutputRoot=fullfile(projectRoot,"matlab","results","physics_mpc_v032");
end
if ~isfolder(opts.OutputRoot), mkdir(opts.OutputRoot); end
fprintf("\n=== Physics-MPC v0.3.2 Smoke ===\n");
...
try
    ...
    report.info=info;
    if ~isfield(info,"version") || ~contains(string(info.version),"v0.3.2")
        error("AirdropX:PhysMPC:OldOracle", ...
            "Physics Oracle is not v0.3.2. Rebuild it once without -SkipBuild before running Smoke.");
    end
    fprintf("Oracle: %s\n",string(info.version));
    ...
    if ~selftest.pass
        ...
        error("AirdropX:PhysMPC:OracleSelfTestFailed", ...
            ...
            selftest.physical_ok,selftest.fuel_frozen,selftest.control_ok,selftest.semigroup.pass,selftest.semigroup.max_scaled);
    end
    ...
    fprintf("=== Physics-MPC v0.3.2 Smoke PASS ===\n\n");
catch ME
...
fprintf("pass_solver=%d (diagnostic) pass_initial=%d pass_acceleration=%d pass_fixed_point=%d pass_height=%d pass_physics=%d pass=%d\n", ...
    t.pass_solver,t.pass_initial,t.pass_acceleration,t.pass_fixed_point,t.pass_height,t.pass_physics,t.pass);
end
