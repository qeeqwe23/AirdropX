...
public:
    ...
    mxArray* info() const
    {
        ...
        mxSetField(s,0,"version",mxCreateString("AirdropX JSBSim physics oracle v0.3.2"));
        return s;
    }
...
private:
                      ...
                      int cfgId, double fuelScale, bool steadyEngine)
    {
        ...
        if (!steadyEngine) {
            ...
            if (!std::isfinite(r1) || !std::isfinite(r2) ||
                ...
                throw std::runtime_error("Engine state changed during zero-dt derivative priming.");
        }
    }
    ...
    void primeCurrentDynamics()
    {
        // After a full model reset and fresh control writes, JSBSim's stored
        // accelerations still correspond to the pre-command zero-dt pass that
        // RunIC() executed earlier. If propagation starts immediately, the first
        // real integration step uses stale derivatives and can create a false
        // theta transient even when q returns to zero at the end of Ts.
        //
        // Reproduce the essential RunIC derivative-priming sequence *after* the
        // final elevator/throttle/engine-state writes: run every scheduled model
        // once with dt=0, then seed FGPropagate's multistep derivative history
        // from the now-current accelerations.
        if (fdm_->IntegrationSuspended())
            throw std::runtime_error("Oracle derivative priming entered with integration already suspended.");
        fdm_->SuspendIntegration();
        try {
            if (!fdm_->Run())
                throw std::runtime_error("JSBSim zero-dt refresh failed while priming current dynamics.");
    ...
    {
        const char* f[] = {"mass_kg","cg_x_m","Iyy_kgm2","alpha_rad","gamma_rad","theta_rad",
        ...
        mxArray* s=mxCreateStructMatrix(1,1,28,f);
...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[])
{
    try {
        ...
        if (cmd == "version") {
            if (nlhs>0) plhs[0]=mxCreateString("AirdropX JSBSim physics oracle v0.3.2");
            return;
        }
