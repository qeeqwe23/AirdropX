function R=airdropx_wind_finalize_v111(outputRoot,manifestPath)
%AIRDROPX_WIND_FINALIZE_V111 Aggregate eight real-JSBSim longitudinal wind tests.
...
R=struct("version","Physics-MPC v1.1.3 longitudinal wind estimation validation","pass",all(Found)&all(Pass),"scenarios_found",sum(Found),"scenarios_pass",sum(Pass),"scenarios_expected",n,"rows",T,"completed_at",datetime("now"));
save(fullfile(outputRoot,"wind_validation.mat"),"R","T");
fid=fopen(fullfile(outputRoot,"wind_validation_summary.txt"),"w"); if fid>=0
    c=onCleanup(@()fclose(fid)); fprintf(fid,"Physics-MPC v1.1.3 longitudinal wind estimation validation\nvalidation_pass=%d\nscenarios_found=%d/%d\nscenarios_pass=%d/%d\n",R.pass,sum(Found),n,sum(Pass),n);
    if any(Found)
...
try
    f=figure("Visible","off","Position",[100 100 1500 850]); tl=tiledlayout(f,2,2,"TileSpacing","compact","Padding","compact"); title(tl,"AirdropX v1.1.3 longitudinal wind sensing validation");
