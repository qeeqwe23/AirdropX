function [s,o]=airdropx_longitudinal_wind_estimator_step_v111(s,Va_mps,Vz_mps,Vg_mps)
%AIRDROPX_LONGITUDINAL_WIND_ESTIMATOR_STEP_V111 One 10-Hz-capable wind observer update.
if ~isstruct(s) || ~isfield(s,"version") || ~contains(string(s.version),"v1.1.1")
    error("AirdropX:Wind:BadEstimatorState","Initialize with airdropx_longitudinal_wind_estimator_init_v111.");
end
s.sample_count=s.sample_count+1;
s.x=s.F*s.x;
s.P=s.F*s.P*s.F.'+s.Q;
m=airdropx_longitudinal_wind_measurement_v111(Va_mps,Vz_mps,Vg_mps, ...
