function report=airdropx_wind_estimation_entry_v110(projectRoot,opts)
%AIRDROPX_WIND_ESTIMATION_ENTRY_V110 Real JSBSim longitudinal wind sensing validation.
arguments
...
try
    ...
    Th=airdropx_wind_simulink_harness_v110(projectRoot,opts.ScenarioName,opts.Duration_s);
    ...
    cmd=airdropx_wind_profile_v110(opts.ScenarioName,te);
    cross=interp1(Th.t_s,Th.wind_cross_truth_mps,te,"linear","extrap");

    s=airdropx_longitudinal_wind_estimator_init_v110(opts.EstimatorTs_s, ...
    ...
    for k=1:n
        [s,o]=airdropx_longitudinal_wind_estimator_step_v110(s,Va(k),Vz(k),Vg(k));
    ...
    for seed=1:opts.MonteCarloSeeds
        ...
        ss=airdropx_longitudinal_wind_estimator_init_v110(opts.EstimatorTs_s, ...
            SigmaGroundspeed_mps=opts.SigmaGroundspeed_mps,SigmaAirspeed_mps=opts.SigmaAirspeed_mps,SigmaVerticalSpeed_mps=opts.SigmaVerticalSpeed_mps);
        ee=zeros(n,1); rr=zeros(n,1); sg=zeros(n,1);
        for k=1:n
            [ss,oo]=airdropx_longitudinal_wind_estimator_step_v110(ss,Van(k),Vzn(k),Vgn(k));
    ...
    report=struct("version","Physics-MPC v1.1.0 longitudinal wind estimator validation","pass",gate.pass,"scenario",opts.ScenarioName,"options",opts,"metrics",metrics,"gate",gate,"completed_at",datetime("now"));
...
m=r.metrics; fprintf(fid,"Physics-MPC v1.1.0 longitudinal wind estimator validation\nscenario=%s\npass=%d\n",r.scenario,r.pass);
...
try
    f=figure("Visible","off","Position",[80 80 1450 900]); tl=tiledlayout(f,3,1,"TileSpacing","compact","Padding","compact"); title(tl,"AirdropX v1.1.0 longitudinal wind estimation - "+name);
