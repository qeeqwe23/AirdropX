function r=airdropx_wind_synthetic_selftest_v110(outputPath)
%AIRDROPX_WIND_SYNTHETIC_SELFTEST_V110 Independent geometry/noise/step preflight.
Ts=0.1; maxGeom=0;
for Va=[45 50 55 60 65]
    for Vz=[-3 -1 0 1 3]
        ...
        for w=-15:5:15
            Vg=Vah+w;
            m=airdropx_longitudinal_wind_measurement_v110(Va,Vz,Vg);
...
for seed=1:nseed
    ...
    s=airdropx_longitudinal_wind_estimator_init_v110(Ts); e=zeros(size(t));
    for k=1:numel(t), [s,o]=airdropx_longitudinal_wind_estimator_step_v110(s,Va(k),Vz(k),Vg(k)); e(k)=o.wind_est_mps; end
    mask=localMask(t,w); er=e-w; rmsv(seed)=sqrt(mean(er(mask).^2)); p95(seed)=localP(abs(er(mask)),95); settle(seed)=localSettle(t,w,e);
end
r=struct("version","Physics-MPC v1.1.0 wind synthetic self-test","geometry_max_error_mps",maxGeom,"mc_rmse_mean_mps",mean(rmsv),"mc_rmse_worst_mps",max(rmsv),"mc_p95_mean_mps",mean(p95),"step_settling90_worst_s",max(settle),"pass",false);
r.pass=maxGeom<=1e-12 && mean(rmsv)<=0.35 && mean(p95)<=0.75 && max(settle)<=0.5;
if nargin>0 && strlength(string(outputPath))>0
    fid=fopen(outputPath,"w"); if fid>=0, c=onCleanup(@()fclose(fid)); fprintf(fid,"Physics-MPC v1.1.0 wind synthetic self-test\n"); fn=fieldnames(r); for i=1:numel(fn), v=r.(fn{i}); if isnumeric(v)&&isscalar(v), fprintf(fid,"%s=%.12g\n",fn{i},v); elseif islogical(v), fprintf(fid,"%s=%d\n",fn{i},v); end, end, end
end
