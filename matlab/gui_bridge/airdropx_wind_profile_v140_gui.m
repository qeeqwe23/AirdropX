...
%AIRDROPX_WIND_PROFILE_V140_GUI Deterministic GUI-only longitudinal wind profile.
% Positive = tailwind, negative = headwind, matching v1.4.0 formal convention.
arguments
    t double
...
switch kind
    ...
    case "ramp"
        tr=max(double(c.ramp_s),eps); a=t>=t0 & t<t0+tr; w(a)=A.*(t(a)-t0)./tr; w(t>=t0+tr)=A;
    case "sine"
        per=max(double(c.period_s),eps); a=t>=t0; w(a)=A.*sin(2*pi*(t(a)-t0)./per);
        % GUI sine also gets a smooth settling tail, but it is engineering mode,
        % not the formal fixed-amplitude sine certification scenario.
    ...
    case "turbulence"
        a=t>=t0; tau=t(a)-t0;
        % Deterministic broadband-like gust. Stateless so carrier and cargo truth
        % see exactly the same wind when queried at arbitrary times.
