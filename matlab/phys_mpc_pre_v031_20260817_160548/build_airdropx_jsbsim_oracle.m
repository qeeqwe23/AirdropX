...
%BUILD_AIRDROPX_JSBSIM_ORACLE Transactional build of Physics Oracle v0.3.1.
...
% than the API used by v0.3.1, instead of surfacing a long compiler error later.
...
localRequireSymbols(fullfile(jsbsimIncludeDir,"models","FGFCS.h"), ...
    ["SetThrottleCmd","SetThrottlePos","GetThrottleCmd","SetDeCmd","GetDeCmd", ...
...
try
    ...
    assert(contains(string(v),"v0.3.1"),"Candidate Oracle returned unexpected version: %s",string(v));
catch ME
    clear airdropx_jsbsim_oracle_mex;
...
try
    ...
    assert(contains(string(vInstalled),"v0.3.1"), ...
...
if ~isempty(missing)
    error("AirdropX:PhysMPC:JSBSimApiMismatch", ...
        "JSBSim header %s lacks required v0.3.1 API symbol(s): %s", ...
