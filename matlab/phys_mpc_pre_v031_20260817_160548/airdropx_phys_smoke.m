...
if opts.OutputRoot==""
    opts.OutputRoot=fullfile(projectRoot,"matlab","results","physics_mpc_v031");
end
if ~isfolder(opts.OutputRoot), mkdir(opts.OutputRoot); end
fprintf("\n=== Physics-MPC v0.3.1 Smoke ===\n");
...
try
    ...
    report.info=info;
    if ~isfield(info,"version") || ~contains(string(info.version),"v0.3.1")
        error("AirdropX:PhysMPC:OldOracle", ...
            "Physics Oracle is not v0.3.1. Rebuild it once without -SkipBuild before running Smoke.");
    end
    fprintf("Oracle: %s\n",string(info.version));
    ...
    if ~trim.pass
        ...
        error("AirdropX:PhysMPC:SmokeTrimFailed", ...
            "Trim failed: exitflag=%d maxScaledRate=%.6g hStep=%.6g m rates=[%.6g %.6g %.6g %.6g %.6g %.6g].", ...
            trim.exitflag,norm(trim.scaled_residual,inf),trim.h_step_m,trim.rate_residual);
    end
    ...
    if ~selftest.pass
        ...
        error("AirdropX:PhysMPC:OracleSelfTestFailed", ...
            ["Oracle self-test failed: deterministic=%d maxSpread=%.3g " + ...
             ...
             "physicalOk=%d fuelFrozen=%d semigroupPass=%d semigroupMaxScaled=%.3g"], ...
            ...
            selftest.physical_ok,selftest.fuel_frozen,selftest.semigroup.pass,selftest.semigroup.max_scaled);
    end
    ...
    fprintf("=== Physics-MPC v0.3.1 Smoke PASS ===\n\n");
catch ME
...
fprintf("pass_solver=%d (diagnostic) pass_initial=%d pass_fixed_point=%d pass_height=%d pass_physics=%d pass=%d\n", ...
    t.pass_solver,t.pass_initial,t.pass_fixed_point,t.pass_height,t.pass_physics,t.pass);
end
