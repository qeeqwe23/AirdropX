...
try
    ...
    if ~isfield(info,"version") || ~contains(string(info.version),"v0.3.0")
        error("AirdropX:PhysMPC:OldOracle", ...
            "Physics Oracle is not v0.3.0. Rebuild it once without -SkipBuild before running Smoke.");
    end
    fprintf("Oracle: %s\n",string(info.version));
    ...
    selftest=airdropx_phys_oracle_selftest(trim.x,trim.u,p);
    ...
    fprintf("=== Physics-MPC v0.3.0 Smoke PASS ===\n\n");
catch ME
