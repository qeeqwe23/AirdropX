...
%BUILD_AIRDROPX_JSBSIM_ORACLE Transactional build of Physics Oracle v0.3.0.
...
% than the API used by v0.3.0, instead of surfacing a long compiler error later.
...
try
    ...
    assert(contains(string(v),"v0.3.0"),"Candidate Oracle returned unexpected version: %s",string(v));
catch ME
    clear airdropx_jsbsim_oracle_mex;
...
try
    ...
    assert(contains(string(vInstalled),"v0.3.0"), ...
...
if ~isempty(missing)
    error("AirdropX:PhysMPC:JSBSimApiMismatch", ...
        "JSBSim header %s lacks required v0.3.0 API symbol(s): %s", ...
