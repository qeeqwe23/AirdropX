...
%
% IMPORTANT: every oracle evaluation first converges JSBSim's internal
% alphadot <-> aerodynamic force <-> acceleration zero-dt loop. The optimizer
% therefore solves the algebraically self-consistent instantaneous equations:
%       udot = 0
%       wdot = 0
%       qdot = 0
...
arguments
    ...
    opts.AlphaDotTol_radps (1,1) double {mustBePositive} = 1e-7
...
alphaDot=raw.diag0.alphadot_radps;
scaledAlphaDot=alphaDot/opts.AlphaDotTol_radps;
algebraicConverged=isfield(raw.diag0,"algebraic_settle_converged") && logical(raw.diag0.algebraic_settle_converged);
...
trim=struct();
trim.method="JSBSim_FGTrim_seed__steady_engine__algebraic_consistent_EOM_polish__discrete_verify";
...
trim.alphadot_residual=alphaDot;
trim.alphadot_tolerance=opts.AlphaDotTol_radps;
trim.scaled_alphadot=scaledAlphaDot;
trim.algebraic_converged=algebraicConverged;
...
trim.pass_alphadot=algebraicConverged && isfinite(scaledAlphaDot) && abs(scaledAlphaDot)<=1;
...
trim.pass_physics=trim.pass_initial && trim.pass_acceleration && trim.pass_alphadot && trim.pass_fixed_point && trim.pass_height;
