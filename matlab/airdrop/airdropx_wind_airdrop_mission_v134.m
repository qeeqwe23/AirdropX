function report=airdropx_wind_airdrop_mission_v134(projectRoot,opts)
%AIRDROPX_WIND_AIRDROP_MISSION_V134 Confidence-gated wind MPC + event-gated energy-aware gust recovery.
%
...
if opts.DisturbanceMinWindConfidence>1 || opts.DisturbanceMinRateConfidence>1, error("AirdropX:WindMPC:BadDisturbanceEvidence","Disturbance confidence thresholds must be in [0,1]."); end
...
if opts.OutputRoot=="", opts.OutputRoot=fullfile(projectRoot,"matlab","results","physics_mpc_v134_stable_energy_recovery_airdrop",opts.ScenarioName+"_"+mode); end
...
try
    ...
    for k=1:Nsim
        ...
        disturbanceEvidence=(WindConfidence(k)>=opts.DisturbanceMinWindConfidence) || (WindRateConfidence(k)>=opts.DisturbanceMinRateConfidence);
        ...
        % Release guidance. Truth wind is prohibited here. v1.3.4 keeps the
        ...
        if cfg<4
            ...
            if SR.release_now
                ...
                fprintf("[V134 DROP] %s %s cargo%d t=%.3f phase=0 target=%.2f release=%.2f pred=%.2f truth=%.2f miss=%+.2f windEst=%+.2f truthWind=%+.2f\n", ...
        ...
        if opts.UseWindDisturbanceMPC
            % v1.3.4: strong recovery is event-gated.  A real abrupt gust must
        ...
        if scheduledRelease
            ...
            fprintf("[V134 DROP] %s %s cargo%d t=%.3f phase=%.3f target=%.2f release=%.2f pred=%.2f truth=%.2f miss=%+.2f windEst=%+.2f truthWind=%+.2f\n", ...
        ...
        if k==1 || DropEvent(k) || mod(k,50)==0
            fprintf("[V134] %s %s t=%5.1f cfg=%d x=%.1f hErr=%+.3f VaErr=%+.3f wind=%+.2f eff=%+.2f truth=%+.2f conf=%.2f rec=%.2f dW1=%+.3f res=%.3g qp=%.2fms\n", ...
    ...
    report=struct("version","Physics-MPC v1.3.4 strict-event-gated stable energy-aware gust-recovery precision airdrop","pass",gate.pass,"scenario",opts.ScenarioName,"mode",mode,"control_mode",controlMode,"release_mode",releaseMode,"oracle_version",vOracle,"options",opts,"metrics",metrics,"gate",gate,"cargo",Cargo,"gust_event_indices",gustEvents,"completed_at",datetime("now"));
...
fprintf(fid,"Physics-MPC v1.3.4 strict-event-gated stable energy-aware gust-recovery precision airdrop\nscenario=%s\nmode=%s\npass=%d\n",r.scenario,r.mode,r.pass);
...
try
    f=figure("Visible","off","Position",[80 80 1550 980]); tl=tiledlayout(f,2,2,"TileSpacing","compact","Padding","compact"); title(tl,"AirdropX v1.3.4 "+scenario+" / "+mode,"Interpreter","none");
