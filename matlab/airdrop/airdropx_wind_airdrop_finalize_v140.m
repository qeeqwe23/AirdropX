function R=airdropx_wind_airdrop_finalize_v140(root,manifestPath)
%AIRDROPX_WIND_AIRDROP_FINALIZE_V140 Aggregate sensor-realistic 3-way comparisons.
arguments
    root (1,1) string
...
for i=1:height(M)
    for mode=modes
        ...
        stateEstP95=NaN; posEstP95=NaN; vgEstP95=NaN; vzEstP95=NaN; realisticAvionics=NaN; independentCargo=NaN;
        if found
...
R=struct(); R.version="Physics-MPC v1.4.0 sensor-realistic transient-evidence energy-aware precision airdrop validation"; R.pass=passIntegrated && allExecuted; R.integrated_pass=passIntegrated; R.all_three_way_executed=allExecuted; R.rows=rows; R.comparison=comparison; R.median_legacy_over_wind_mpc_carrier_rms=medianCarrierRatio; R.completed_at=datetime("now");
...
writetable(rows,fullfile(root,"wind_sensor_realism_validation.csv")); writetable(comparison,fullfile(root,"wind_sensor_realism_comparison.csv")); save(fullfile(root,"wind_sensor_realism_validation.mat"),"R","-v7.3");
localWrite(R,fullfile(root,"wind_sensor_realism_validation_summary.txt")); localPlot(comparison,root);
end
function localWrite(R,path)
fid=fopen(path,"w"); if fid<0, return; end; c=onCleanup(@()fclose(fid)); %#ok<NASGU>
fprintf(fid,"Physics-MPC v1.4.0 sensor-realistic transient-evidence energy-aware precision airdrop validation\nvalidation_pass=%d\nintegrated_wind_mpc_scenarios_pass=%d/8\nall_three_way_missions_found=%d/24\n",R.pass,sum(R.rows.Pass & R.rows.Mode=="wind_mpc_aware"),sum(R.rows.Found));
...
try
    ...
    nexttile; bar(categorical(C.Scenario),[C.WindMpcPostWindRms C.LegacyPostWindRms]); ylabel("post-wind primary RMS"); legend("v1.4.0 sensor-realistic MPC","legacy MPC"); grid on;
    nexttile; bar(categorical(C.Scenario),[C.WindMpcRecovery_s C.LegacyRecovery_s]); ylabel("gust recovery (s)"); legend("v1.4.0","legacy"); grid on;
    ...
    exportgraphics(f,fullfile(root,"wind_sensor_realism_validation.png"),"Resolution",160); close(f);
catch ME
