function out=airdropx_airdrop_truth_impact_v121(scenarioName,release,opts)
%AIRDROPX_AIRDROP_TRUTH_IMPACT_V121 Scoring-only cargo truth using actual wind profile.
% The release controller does not call this until after a release decision.
arguments
...
windFun=@(tau) double(airdropx_wind_profile_v111(scenarioName,double(release.t_s)+tau));
out=localIntegrate(double(release.x_m),double(release.h_m),double(release.vx_ground_mps),double(release.vz_up_mps),windFun,p);
out.mode="scoring_only_true_wind_profile";
end
