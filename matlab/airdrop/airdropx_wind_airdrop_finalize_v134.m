function R=airdropx_wind_airdrop_finalize_v134(root,manifestPath)
%AIRDROPX_WIND_AIRDROP_FINALIZE_V134 Aggregate v1.3.4 3-way comparisons and recovery/energy diagnostics.
arguments
    root (1,1) string
...
for i=1:height(M)
    for mode=modes
        ...
        abruptCount=NaN; energyActive=NaN; energyShift=NaN; inputSnap=NaN;
        if found
...
R=struct(); R.version="Physics-MPC v1.3.4 strict-event-gated stable energy-aware gust-recovery precision airdrop validation"; R.pass=passIntegrated && allExecuted; R.integrated_pass=passIntegrated; R.all_three_way_executed=allExecuted; R.rows=rows; R.comparison=comparison; R.median_legacy_over_wind_mpc_carrier_rms=medianCarrierRatio; R.completed_at=datetime("now");
...
writetable(rows,fullfile(root,"wind_stable_energy_recovery_validation.csv")); writetable(comparison,fullfile(root,"wind_stable_energy_recovery_comparison.csv")); save(fullfile(root,"wind_stable_energy_recovery_validation.mat"),"R","-v7.3");
localWrite(R,fullfile(root,"wind_stable_energy_recovery_validation_summary.txt")); localPlot(comparison,root);
end
function localWrite(R,path)
fid=fopen(path,"w"); if fid<0, return; end; c=onCleanup(@()fclose(fid)); %#ok<NASGU>
fprintf(fid,"Physics-MPC v1.3.4 strict-event-gated stable energy-aware gust-recovery precision airdrop validation\nvalidation_pass=%d\nintegrated_wind_mpc_scenarios_pass=%d/8\nall_three_way_missions_found=%d/24\n",R.pass,sum(R.rows.Pass & R.rows.Mode=="wind_mpc_aware"),sum(R.rows.Found));
...
for i=1:height(R.comparison), q=R.comparison(i,:); fprintf(fid,"%s carrierRms windMPC=%.6g legacy=%.6g ratio=%.6g recovery windMPC=%.6g legacy=%.6g residual[0.5/1/3]=%.6g/%.6g/%.6g tail=%.6g recoveryActive=%.4g throttleSat=%.4g elevatorSat=%.4g abrupt=%g energyActive=%.4g energyShift=%.4g snap=%.3g landingAware=%.6g noWind=%.6g\n",char(q.Scenario),q.WindMpcPostWindRms,q.LegacyPostWindRms,q.LegacyOverWindMpcCarrierRms,q.WindMpcRecovery_s,q.LegacyRecovery_s,q.Residual0p5Norm,q.Residual1p0Norm,q.Residual3p0Norm,q.Tail5RmsNorm,q.RecoveryActiveFraction,q.ThrottleSatFraction,q.ElevatorSatFraction,q.AbruptGustTriggerCount,q.EnergyRecoveryActiveFraction,q.EnergyAltitudeShiftPeak_m,q.InputSnapMax,q.AwareLandingRms_m,q.NoWindReleaseLandingRms_m); end
end
...
try
    ...
    nexttile; bar(categorical(C.Scenario),[C.WindMpcPostWindRms C.LegacyPostWindRms]); ylabel("post-wind primary RMS"); legend("v1.3.4 stable energy/recovery MPC","legacy MPC"); grid on;
    nexttile; bar(categorical(C.Scenario),[C.WindMpcRecovery_s C.LegacyRecovery_s]); ylabel("gust recovery (s)"); legend("v1.3.4","legacy"); grid on;
    ...
    exportgraphics(f,fullfile(root,"wind_stable_energy_recovery_validation.png"),"Resolution",160); close(f);
catch ME
