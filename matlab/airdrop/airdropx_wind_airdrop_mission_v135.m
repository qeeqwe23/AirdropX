function report=airdropx_wind_airdrop_mission_v135(projectRoot,opts)
%AIRDROPX_WIND_AIRDROP_MISSION_V135 Evidence-latched wind MPC + auditable energy-aware gust recovery.
%
...
arguments
    ...
    % v1.3.5: rate confidence alone may EXTEND an already-proven wind event,
    % but may not activate the disturbance MPC from calm sensor noise.
    opts.DisturbanceActivationWindConfidence (1,1) double {mustBeNonnegative} = 0.25
    opts.DisturbanceReleaseWindConfidence (1,1) double {mustBeNonnegative} = 0.08
    opts.DisturbanceActivationHold_s (1,1) double {mustBePositive} = 0.30
    opts.DisturbancePersistence_s (1,1) double {mustBePositive} = 0.80
    opts.DisturbanceRateExtendConfidence (1,1) double {mustBeNonnegative} = 0.70
...
if opts.DisturbanceMinWindConfidence>1 || opts.DisturbanceMinRateConfidence>1 || opts.DisturbanceActivationWindConfidence>1 || opts.DisturbanceReleaseWindConfidence>1 || opts.DisturbanceRateExtendConfidence>1, error("AirdropX:WindMPC:BadDisturbanceEvidence","Disturbance confidence thresholds must be in [0,1]."); end
if opts.DisturbanceReleaseWindConfidence>opts.DisturbanceActivationWindConfidence, error("AirdropX:WindMPC:BadDisturbanceHysteresis","Release wind confidence must not exceed activation confidence."); end
...
if opts.OutputRoot=="", opts.OutputRoot=fullfile(projectRoot,"matlab","results","physics_mpc_v135_equivalent_energy_recovery_airdrop",opts.ScenarioName+"_"+mode); end
...
try
    ...
    w0=double(airdropx_wind_profile_v135(opts.ScenarioName,0,SineForcingEnd_s=opts.SineForcingEnd_s,SineSettleRamp_s=opts.SineSettleRamp_s));
    ...
    AbruptGustTrigger=false(Nsim,1); DisturbanceEvidenceActive=false(Nsim,1); EnergyHrefShift=nan(Nsim,1); InputSnapMagnitude=nan(Nsim,1);
    ...
    disturbanceActive=false; disturbanceStrongCount=0; disturbanceHoldCount=0;
    ...
    for k=1:Nsim
        ...
        % v1.3.5 disturbance-evidence latch.  v1.3.4 used
        %   windConfidence >= threshold OR rateConfidence >= threshold,
        % which let zero-wind rate noise invoke the disturbance QP for a large
        % fraction of calm flight.  Activation now requires persistent credible
        % WIND AMPLITUDE, or a separately qualified abrupt gust.  Rate evidence
        % can only keep an already-established event alive across a zero crossing.
        deltaWindEst=0;
        ...
        if WindConfidence(k)>=opts.DisturbanceActivationWindConfidence
            disturbanceStrongCount=disturbanceStrongCount+1;
        else
            disturbanceStrongCount=0;
        end
        activateN=max(1,ceil(opts.DisturbanceActivationHold_s/Ts));
        persistN=max(1,ceil(opts.DisturbancePersistence_s/Ts));
        if gustTrigger || disturbanceStrongCount>=activateN
            disturbanceActive=true; disturbanceHoldCount=persistN;
        elseif disturbanceActive
            if WindConfidence(k)>=opts.DisturbanceReleaseWindConfidence || WindRateConfidence(k)>=opts.DisturbanceRateExtendConfidence
                disturbanceHoldCount=persistN;
            else
                disturbanceHoldCount=max(0,disturbanceHoldCount-1);
                if disturbanceHoldCount==0, disturbanceActive=false; end
            end
        end
        DisturbanceEvidenceActive(k)=disturbanceActive;
        disturbanceEvidence=disturbanceActive;
        ...
        % Release guidance. Truth wind is prohibited here. v1.3.5 keeps the
        ...
        if cfg<4
            ...
            if SR.release_now
                ...
                truth=airdropx_airdrop_truth_impact_v135(opts.ScenarioName,truthRelease,SineForcingEnd_s=opts.SineForcingEnd_s,SineSettleRamp_s=opts.SineSettleRamp_s);
                ...
                fprintf("[V135 DROP] %s %s cargo%d t=%.3f phase=0 target=%.2f release=%.2f pred=%.2f truth=%.2f miss=%+.2f windEst=%+.2f truthWind=%+.2f\n", ...
        ...
        if opts.UseWindDisturbanceMPC
            % v1.3.5: strong recovery is event-gated.  A real abrupt gust must
            ...
            if ((~disturbanceEvidence && ~gustLatched) || gNorm<=opts.DisturbanceSolveDeadbandNormalized) && selectedLevel==0
                % No credible external disturbance (or numerically negligible
                % gSeq) uses the original certified solver exactly.  This makes
                % calm control invariant to estimator/residual noise.
        ...
        if scheduledRelease
            ...
            if tau>1e-9
                wCmd1=double(airdropx_wind_profile_v135(opts.ScenarioName,t(k),SineForcingEnd_s=opts.SineForcingEnd_s,SineSettleRamp_s=opts.SineSettleRamp_s));
            ...
            if rem>1e-9
                wCmd2=double(airdropx_wind_profile_v135(opts.ScenarioName,tRel,SineForcingEnd_s=opts.SineForcingEnd_s,SineSettleRamp_s=opts.SineSettleRamp_s));
            ...
            fprintf("[V135 DROP] %s %s cargo%d t=%.3f phase=%.3f target=%.2f release=%.2f pred=%.2f truth=%.2f miss=%+.2f windEst=%+.2f truthWind=%+.2f\n", ...
            ...
            wCmd=double(airdropx_wind_profile_v135(opts.ScenarioName,t(k),SineForcingEnd_s=opts.SineForcingEnd_s,SineSettleRamp_s=opts.SineSettleRamp_s));
        ...
        if k==1 || DropEvent(k) || mod(k,50)==0
            fprintf("[V135] %s %s t=%5.1f cfg=%d x=%.1f hErr=%+.3f VaErr=%+.3f wind=%+.2f eff=%+.2f truth=%+.2f conf=%.2f rec=%.2f dW1=%+.3f res=%.3g qp=%.2fms\n", ...
    ...
    if isSine
        ...
        % gates after the v1.3.5 wind profile has smoothly returned to zero.
    ...
    report=struct("version","Physics-MPC v1.3.5 base-equivalent evidence-latched energy-aware gust-recovery precision airdrop","pass",gate.pass,"scenario",opts.ScenarioName,"mode",mode,"control_mode",controlMode,"release_mode",releaseMode,"oracle_version",vOracle,"options",opts,"metrics",metrics,"gate",gate,"cargo",Cargo,"gust_event_indices",gustEvents,"completed_at",datetime("now"));
...
fprintf(fid,"Physics-MPC v1.3.5 base-equivalent evidence-latched energy-aware gust-recovery precision airdrop\nscenario=%s\nmode=%s\npass=%d\n",r.scenario,r.mode,r.pass);
...
try
    f=figure("Visible","off","Position",[80 80 1550 980]); tl=tiledlayout(f,2,2,"TileSpacing","compact","Padding","compact"); title(tl,"AirdropX v1.3.5 "+scenario+" / "+mode,"Interpreter","none");
