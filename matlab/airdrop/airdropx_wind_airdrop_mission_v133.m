function report=airdropx_wind_airdrop_mission_v133(projectRoot,opts)
%AIRDROPX_WIND_AIRDROP_MISSION_V133 Confidence-gated wind MPC + event-gated energy-aware gust recovery.
%
...
arguments
    ...
    opts.ContinuousAssistMax (1,1) double {mustBeNonnegative} = 0.25
    ...
    opts.EnergyMaxAltitudeShift_m (1,1) double {mustBeNonnegative} = 2.5
...
if opts.OutputRoot=="", opts.OutputRoot=fullfile(projectRoot,"matlab","results","physics_mpc_v133_energy_recovery_airdrop",opts.ScenarioName+"_"+mode); end
...
try
    ...
    if opts.UseWindDisturbanceMPC
        ...
        if opts.UseUnifiedGustRecovery
            sched=airdropx_phys_mpc_build_recovery_bank_v133(sched);
        end
    end
    ...
    AbruptGustTrigger=false(Nsim,1); EnergyHrefShift=nan(Nsim,1); InputSnapMagnitude=nan(Nsim,1);
    ...
    for k=1:Nsim
        ...
        % Release guidance. Truth wind is prohibited here. v1.3.3 keeps the
        ...
        if cfg<4
            ...
            if SR.release_now
                ...
                fprintf("[V133 DROP] %s %s cargo%d t=%.3f phase=0 target=%.2f release=%.2f pred=%.2f truth=%.2f miss=%+.2f windEst=%+.2f truthWind=%+.2f\n", ...
        ...
        if opts.UseWindDisturbanceMPC
            % v1.3.3: strong recovery is event-gated.  A real abrupt gust must
            ...
            % Gw preview plus at most a weak assist; estimator noise cannot latch
            ...
            assist=0;
            if WindRateConfidence(k)>=opts.ContinuousAssistMinRateConfidence
            ...
            if gNorm<=opts.DisturbanceSolveDeadbandNormalized && selectedLevel==0
                % Near-zero wind/residual evidence uses the original certified
                % solver exactly; this prevents calm sensor noise from creating
                % a permanent disturbance-control path.
            ...
            if opts.UseEnergyRecovery && gustLatched && selectedLevel>=0.5 && sol.feasible
                ...
                if (vaSignedN<=-opts.EnergyVaOnsetNormalized && rawThrottle>=opts.EnergyThrottleHigh) || ...
                   (vaSignedN>= opts.EnergyVaOnsetNormalized && rawThrottle<=opts.EnergyThrottleLow)
                    ...
                    energyDesired=opts.EnergyMaxAltitudeShift_m*sign(vaSignedN)*amp*max(selectedLevel,0.5);
                end
            end
            ...
            energyHrefState=max(min(energyHrefState,opts.EnergyMaxAltitudeShift_m),-opts.EnergyMaxAltitudeShift_m);
        ...
        if scheduledRelease
            ...
            fprintf("[V133 DROP] %s %s cargo%d t=%.3f phase=%.3f target=%.2f release=%.2f pred=%.2f truth=%.2f miss=%+.2f windEst=%+.2f truthWind=%+.2f\n", ...
        ...
        if k==1 || DropEvent(k) || mod(k,50)==0
            fprintf("[V133] %s %s t=%5.1f cfg=%d x=%.1f hErr=%+.3f VaErr=%+.3f wind=%+.2f eff=%+.2f truth=%+.2f conf=%.2f rec=%.2f dW1=%+.3f res=%.3g qp=%.2fms\n", ...
    ...
    report=struct("version","Physics-MPC v1.3.3 event-gated energy-aware gust-recovery precision airdrop","pass",gate.pass,"scenario",opts.ScenarioName,"mode",mode,"control_mode",controlMode,"release_mode",releaseMode,"oracle_version",vOracle,"options",opts,"metrics",metrics,"gate",gate,"cargo",Cargo,"gust_event_indices",gustEvents,"completed_at",datetime("now"));
...
fprintf(fid,"Physics-MPC v1.3.3 event-gated energy-aware gust-recovery precision airdrop\nscenario=%s\nmode=%s\npass=%d\n",r.scenario,r.mode,r.pass);
...
try
    f=figure("Visible","off","Position",[80 80 1550 980]); tl=tiledlayout(f,2,2,"TileSpacing","compact","Padding","compact"); title(tl,"AirdropX v1.3.3 "+scenario+" / "+mode);
