function report=airdropx_wind_airdrop_mission_v136p(projectRoot,opts)
%AIRDROPX_WIND_AIRDROP_MISSION_V136P Paper-validation fair-sensor MPC + transient-evidence gust recovery.
%
% Plant truth: persistent nonlinear JSBSim wind Oracle.
% Onboard state/control: paper-validation unbiased sensor layer (GNSS/INS, baro, pitot, IMU/AHRS, engine telemetry).
% JSBSim truth is prohibited from the online controller/release path except as sensor stimulus.
...
arguments
    ...
    opts.SigmaGroundspeed_mps (1,1) double {mustBeNonnegative} = 0.10
    opts.SigmaAirspeed_mps (1,1) double {mustBeNonnegative} = 0.15
    opts.SigmaVerticalSpeed_mps (1,1) double {mustBeNonnegative} = 0.07
    opts.UsePaperSensorModel (1,1) logical = true % formal paper path; false only for ideal-state ablation
    opts.SigmaGnssPosition_m (1,1) double {mustBeNonnegative} = 0.30
    opts.SigmaBaroAltitude_m (1,1) double {mustBeNonnegative} = 0.20
    opts.SigmaPitch_deg (1,1) double {mustBeNonnegative} = 0.03
    opts.SigmaPitchRate_dps (1,1) double {mustBeNonnegative} = 0.01
    opts.SigmaEngineN1 (1,1) double {mustBeNonnegative} = 0.05
    opts.SigmaEngineN2 (1,1) double {mustBeNonnegative} = 0.05
    opts.UseIndependentCargoTruth (1,1) logical = false
    opts.CargoTruthCdAScale (1,1) double {mustBePositive} = 1.0
...
if opts.OutputRoot=="", opts.OutputRoot=fullfile(projectRoot,"matlab","results","physics_mpc_v136p_paper_airdrop",opts.ScenarioName+"_"+mode); end
...
try
    ...
    xTruth=models{1}.ctrl.xref; uTrim=models{1}.ctrl.uref;
    ...
    D=airdropx_jsbsim_wind_oracle_mex("start_continuous",xTruth,uTrim,0,opts.FuelScale,w0);
    ...
    avionics=airdropx_paper_sensor_init_v136p(Ts,SigmaGnssPos_m=opts.SigmaGnssPosition_m,SigmaGnssGroundspeed_mps=opts.SigmaGroundspeed_mps, ...
        SigmaGnssVerticalSpeed_mps=opts.SigmaVerticalSpeed_mps,SigmaBaroAltitude_m=opts.SigmaBaroAltitude_m,SigmaAirspeed_mps=opts.SigmaAirspeed_mps, ...
        SigmaPitch_rad=deg2rad(opts.SigmaPitch_deg),SigmaPitchRate_radps=deg2rad(opts.SigmaPitchRate_dps),SigmaN1=opts.SigmaEngineN1,SigmaN2=opts.SigmaEngineN2);
    Nsim=ceil(opts.Duration_s/Ts)+1; t=(0:Nsim-1)'*Ts;
    X=nan(Nsim,7); XEst=nan(Nsim,7); U=nan(Nsim,2); ErrTruth=nan(Nsim,7); ErrEst=nan(Nsim,7); Cfg=nan(Nsim,1); PosN=nan(Nsim,1); PosNEst=nan(Nsim,1); Vg=nan(Nsim,1); Vz=nan(Nsim,1); VgNav=nan(Nsim,1); VzNav=nan(Nsim,1);
    ...
    release=struct('cargo',(1:4)','target_m',targets,'released',false(4,1),'t_s',nan(4,1),'release_x_m',nan(4,1), ...
        'release_h_m',nan(4,1),'release_vg_mps',nan(4,1),'release_vz_mps',nan(4,1), ...
        'truth_release_x_m',nan(4,1),'truth_release_h_m',nan(4,1),'truth_release_vg_mps',nan(4,1),'truth_release_vz_mps',nan(4,1), ...
        'wind_est_mps',nan(4,1),'wind_rate_est_mps2',nan(4,1),'wind_truth_mps',nan(4,1),'predicted_impact_m',nan(4,1),'truth_impact_m',nan(4,1),'predicted_error_m',nan(4,1),'landing_error_m',nan(4,1), ...
    ...
    for k=1:Nsim
        ...
        % ---------------- PLANT/TRUTH SIDE ----------------
        % JSBSim truth is stored for scoring and is used only as stimulus to the
        % simulated avionics sensors.  It must not be passed directly to MPC or
        % release guidance.
        X(k,:)=xTruth.'; Cfg(k)=cfg; PosN(k)=double(D.pos_n_m); Vg(k)=double(D.v_north_mps); Vz(k)=double(D.vz_up_mps);
        WindTrue(k)=double(D.wind_long_mps); Mass(k)=double(D.mass_kg); Cg(k)=double(D.cg_x_m); Iyy(k)=double(D.Iyy_kgm2);
        sensorTruth=struct("pos_n_m",PosN(k),"h_m",xTruth(1),"Va_mps",xTruth(2),"theta_rad",xTruth(4),"q_radps",xTruth(5), ...
            "N1",xTruth(6),"N2",xTruth(7),"Vg_long_mps",Vg(k),"Vz_up_mps",Vz(k));
        if opts.UsePaperSensorModel
            [avionics,av]=airdropx_paper_sensor_step_v136p(avionics,sensorTruth);
            xCtrl=av.x_est; posCtrl=av.pos_n_m; VaMeas=av.Va_mps; VgMeas=av.Vg_long_mps; VzMeas=av.Vz_up_mps;
        else
            % Explicit legacy/SIL diagnostic only. Never use this mode to claim
            % paper baseline performance.
            xCtrl=xTruth; posCtrl=PosN(k); VaMeas=xTruth(2)+opts.SigmaAirspeed_mps*randn; VzMeas=Vz(k)+opts.SigmaVerticalSpeed_mps*randn; VgMeas=Vg(k)+opts.SigmaGroundspeed_mps*randn;
        end
        XEst(k,:)=xCtrl.'; PosNEst(k)=posCtrl; VgNav(k)=VgMeas; VzNav(k)=VzMeas;
        % ---------------- ONBOARD/CAUSAL SIDE ----------------
        ...
        % v1.3.6-Paper retains the v1.3.6 split between two meanings of "wind evidence":
        ...
        dxCurrent=airdropx_phys_mpc_state_error(xCtrl,ctrl.xref);
        ...
        % Release guidance. Truth wind is prohibited here. v1.3.6-Paper keeps the
        ...
        if cfg<4
            ...
            rs=struct("x_m",posCtrl,"h_m",xCtrl(1),"vx_ground_mps",vxGuide,"vz_up_mps",GuideVz(k),"wind_est_mps",wGuide,"wind_rate_est_mps2",rGuide);
            ...
            if SR.release_now
                ...
                release.release_x_m(j)=posCtrl; release.release_h_m(j)=xCtrl(1); release.release_vg_mps(j)=VgMeas; release.release_vz_mps(j)=VzMeas;
                release.truth_release_x_m(j)=PosN(k); release.truth_release_h_m(j)=xTruth(1); release.truth_release_vg_mps(j)=Vg(k); release.truth_release_vz_mps(j)=Vz(k);
                ...
                truthRelease=struct("x_m",PosN(k),"h_m",xTruth(1),"vx_ground_mps",Vg(k),"vz_up_mps",Vz(k),"t_s",t(k));
                if opts.UseIndependentCargoTruth
                    truth=airdropx_cargo_truth_plant_v136p(opts.ScenarioName,truthRelease,SineForcingEnd_s=opts.SineForcingEnd_s,SineSettleRamp_s=opts.SineSettleRamp_s,CdAScale=opts.CargoTruthCdAScale);
                else
                ...
                fprintf("[V136P DROP] %s %s cargo%d t=%.3f phase=0 target=%.2f release=%.2f pred=%.2f truth=%.2f miss=%+.2f windEst=%+.2f truthWind=%+.2f\n", ...
                    opts.ScenarioName,mode,j,t(k),targets(j),release.release_x_m(j),release.predicted_impact_m(j),truth.impact_x_m,release.landing_error_m(j),WindEst(k),WindTrue(k));
        ...
        dx=airdropx_phys_mpc_state_error(xCtrl,baseCtrl.xref); ErrEst(k,:)=dx.';
        dxTruth=airdropx_phys_mpc_state_error(xTruth,baseCtrl.xref); ErrTruth(k,:)=dxTruth.';
        if opts.UseWindDisturbanceMPC
            % v1.3.6-Paper: strong recovery remains event-gated. A real abrupt gust must
            ...
            if ~useTransientSolver
                ...
                gSeq=zeros(baseCtrl.n,baseCtrl.N); sol=airdropx_phys_mpc_solve(baseCtrl,xCtrl,warm); WindMpcActive(k)=false; ctrl=baseCtrl;
            else
                ...
                sol=airdropx_phys_mpc_solve_disturbance_v130(ctrl,xCtrl,gSeq,warm); WindMpcActive(k)=true;
            end
            ...
            if opts.UseEnergyRecovery && selectedLevel>0 && abs(energyHrefState)>=1e-4
                ...
                solEnergy=airdropx_phys_mpc_solve_disturbance_v130(ctrlEnergy,xCtrl,gSeq,warm);
            ...
            sol=airdropx_phys_mpc_solve(ctrl,xCtrl,warm);
        ...
        if scheduledRelease
            ...
            if tau>1e-9
                ...
                [xRelTruth,DRel]=airdropx_jsbsim_wind_oracle_mex("step_continuous",u,cfgBeforeStep,opts.FuelScale,wCmd1,tau);
            else
                xRelTruth=xTruth; DRel=D;
            end
            ...
            % No sensor sample arrives inside the 0.1 s control interval. Store
            % the causal extrapolated onboard release state separately from plant truth.
            release.release_x_m(j)=rs.x_m+rs.vx_ground_mps*tau; release.release_h_m(j)=max(0,rs.h_m+rs.vz_up_mps*tau); release.release_vg_mps(j)=rs.vx_ground_mps; release.release_vz_mps(j)=rs.vz_up_mps;
            release.truth_release_x_m(j)=double(DRel.pos_n_m); release.truth_release_h_m(j)=xRelTruth(1); release.truth_release_vg_mps(j)=double(DRel.v_north_mps); release.truth_release_vz_mps(j)=double(DRel.vz_up_mps);
            ...
            % Re-evaluate the causal impact predictor at the scheduled fractional
            % release time using ONLY the onboard state that was knowable before
            % the timer fired. The scheduler residual is logged separately.
            ...
            truthRelease=struct("x_m",release.truth_release_x_m(j),"h_m",release.truth_release_h_m(j),"vx_ground_mps",release.truth_release_vg_mps(j),"vz_up_mps",release.truth_release_vz_mps(j),"t_s",tRel);
            if opts.UseIndependentCargoTruth
                truth=airdropx_cargo_truth_plant_v136p(opts.ScenarioName,truthRelease,SineForcingEnd_s=opts.SineForcingEnd_s,SineSettleRamp_s=opts.SineSettleRamp_s,CdAScale=opts.CargoTruthCdAScale);
            else
                truth=airdropx_airdrop_truth_impact_v136(opts.ScenarioName,truthRelease,SineForcingEnd_s=opts.SineForcingEnd_s,SineSettleRamp_s=opts.SineSettleRamp_s);
            ...
            if rem>1e-9
                ...
                [xNextTruth,DNext]=airdropx_jsbsim_wind_oracle_mex("step_continuous",u,cfg,opts.FuelScale,wCmd2,rem);
            else
                xNextTruth=xRelTruth; DNext=DRel;
            end
            ...
            fprintf("[V136P DROP] %s %s cargo%d t=%.3f phase=%.3f target=%.2f release=%.2f pred=%.2f truth=%.2f miss=%+.2f windEst=%+.2f truthWind=%+.2f\n", ...
            ...
            [xNextTruth,DNext]=airdropx_jsbsim_wind_oracle_mex("step_continuous",u,cfg,opts.FuelScale,wCmd,Ts);
            ep=airdropx_phys_mpc_state_error(xNextTruth,xPred); PredErrInf(k)=max(abs(ep)./ctrl.stateScale);
            prev=struct("ctrl",baseCtrl,"dx",dx,"du",duApplied,"wind_effective",WindEffective(k),"cfg",cfg);
            xTruth=xNextTruth; D=DNext; warm=airdropx_phys_mpc_shift_warmstart(sol.U,ctrl.m,ctrl.N);
        end
        if k==1 || DropEvent(k) || mod(k,50)==0
            fprintf("[V136P] %s %s t=%5.1f cfg=%d x=%.1f hErr=%+.3f VaErr=%+.3f wind=%+.2f eff=%+.2f truth=%+.2f conf=%.2f rec=%.2f dW1=%+.3f res=%.3g qp=%.2fms\n", ...
                opts.ScenarioName,mode,t(k),cfg,posCtrl,dx(1),dx(2),WindEst(k),WindEffective(k),WindTrue(k),WindConfidence(k),RecoveryLevel(k),ForecastDw1(k),ResidualNorm(k),1e3*sol.solve_time_s);
        end
    end

    valid=~isnan(QPExit); finalCtrl=models{cfg+1}.ctrl; dxFinal=airdropx_phys_mpc_state_error(xTruth,finalCtrl.xref);
    primarySeries=nan(Nsim,1); primarySeries(valid)=max(abs(ErrTruth(valid,1:5))./sched.stateScale(1:5).',[],2);
    ...
    impactPredictionError=release.truth_impact_m-release.predicted_impact_m;
    metrics.max_impact_prediction_error_m=max(abs(impactPredictionError),[],'omitnan'); metrics.rms_impact_prediction_error_m=sqrt(mean(impactPredictionError.^2,'omitnan'));
    % Historical field below is the release-to-target residual of the onboard predictor,
    % not the true predictor-model error. Keep it for backward compatibility.
    ...
    metrics.final_normalized_inf=max(abs(dxFinal)./finalCtrl.stateScale); metrics.tail5s_normalized_rms=max(sqrt(mean((ErrTruth(tailIdx,:)./sched.stateScale.').^2,1)));
    ...
    stateEstNorm=max(abs(XEst-X)./sched.stateScale.',[],2);
    metrics.onboard_state_estimation_p95_normalized=localPercentile(stateEstNorm(valid),95);
    metrics.onboard_position_error_p95_m=localPercentile(abs(PosNEst(valid)-PosN(valid)),95);
    metrics.onboard_groundspeed_error_p95_mps=localPercentile(abs(VgNav(valid)-Vg(valid)),95);
    metrics.onboard_vertical_speed_error_p95_mps=localPercentile(abs(VzNav(valid)-Vz(valid)),95);
    metrics.paper_sensor_model_enabled=opts.UsePaperSensorModel; metrics.independent_cargo_truth_enabled=opts.UseIndependentCargoTruth;
    metrics.jsbsim_state_truth_used_by_controller=0; metrics.jsbsim_navigation_truth_used_by_release=0; metrics.truth_wind_used_by_online_guidance=0;
    cp=airdropx_cargo_truth_params_v136p(); metrics.independent_cargo_truth_CdA_m2=cp.CdA_m2; metrics.independent_cargo_truth_calibration_error_m=cp.calibration_error_m;

    % Two result layers are reported. `paper_core` validates a fair experimental
    % setup and numerical closed loop; performance is compared quantitatively in
    % the finalizer. `engineering_gate` preserves the old strict 3 s/1.25 stress
    % thresholds as a secondary diagnostic and is NOT the paper's sole verdict.
    gate=struct(); gate.paper_sensor_model=opts.UsePaperSensorModel; gate.truth_not_used_for_release=1; gate.truth_not_used_by_controller=1; gate.four_drops=metrics.drops_completed==4; gate.wind_estimation=metrics.wind_error_p95_mps<=opts.MaxWindErrorP95_mps;
    ...
    if isSine
        ...
        % gates after the wind profile has smoothly returned to zero.
    ...
    paper_core=struct();
    paper_core.paper_sensor_model=opts.UsePaperSensorModel;
    paper_core.truth_not_used_for_release=true; paper_core.truth_not_used_by_controller=true;
    paper_core.four_drops=metrics.drops_completed==4; paper_core.qp_all_feasible=all(QPExit(valid)>0);
    paper_core.input_bounds=all(U(valid,1)>=-1-1e-9 & U(valid,1)<=1+1e-9 & U(valid,2)>=-1e-9 & U(valid,2)<=1+1e-9);
    paper_core.realtime=metrics.qp_p95_ms<=100;
    paper_core.mass_configuration=metrics.mass_match_error_max_kg<=1e-2 && metrics.cg_match_error_max_m<=1e-5 && metrics.Iyy_match_error_max_kgm2<=1e-2;
    paper_core.wind_estimation=metrics.wind_error_p95_mps<=opts.MaxWindErrorP95_mps;
    pv=struct2cell(paper_core); paper_core.pass=all(cellfun(@(z)logical(z),pv));
    ...
    Cargo=table(release.cargo,release.target_m,release.released,release.t_s,release.release_x_m,release.release_h_m,release.release_vg_mps,release.release_vz_mps, ...
        release.truth_release_x_m,release.truth_release_h_m,release.truth_release_vg_mps,release.truth_release_vz_mps, ...
        release.wind_est_mps,release.wind_rate_est_mps2,release.wind_truth_mps,release.predicted_impact_m,release.truth_impact_m,release.predicted_error_m,impactPredictionError,release.landing_error_m,release.predicted_fall_time_s,release.truth_fall_time_s,release.release_phase_s,release.fractional_release,release.scheduler_residual_m,release.scheduler_mode, ...
        'VariableNames',{'cargo','target_m','released','release_t_s','release_x_est_m','release_h_est_m','release_vg_est_mps','release_vz_est_mps','release_x_truth_m','release_h_truth_m','release_vg_truth_mps','release_vz_truth_mps','wind_est_mps','wind_rate_est_mps2','wind_truth_mps','predicted_impact_m','truth_impact_m','release_target_residual_m','impact_prediction_error_m','landing_error_m','predicted_fall_time_s','truth_fall_time_s','release_phase_s','fractional_release','scheduler_residual_m','scheduler_mode'});
    T=table(t,Cfg,PosN,PosNEst,X(:,1),XEst(:,1),X(:,2),XEst(:,2),X(:,3),XEst(:,3),X(:,4),XEst(:,4),X(:,5),XEst(:,5),X(:,6),XEst(:,6),X(:,7),XEst(:,7),Vg,VgNav,Vz,VzNav,WindTrue,WindRaw,WindEst,WindRate,WindSigma,WindConfidence,WindRateConfidence,WindEffective,WindRateEffective,GuideVg,GuideVa,GuideVz,ForecastDw1,ResidualNorm,WindMpcActive,RecoverySeverity,RecoveryLevel,GustRecoveryLatched,AbruptGustTrigger,ReleaseWindEvidenceActive,CarrierTransientEvidenceActive,DisturbanceEvidenceActive,EnergyHrefShift,InputSnapMagnitude,CurrentTarget,PredImpact,DropEvent,ReleaseIndex,U(:,1),U(:,2),QPExit,QPTime,PredErrInf,primarySeries,causalMask,ReleasePhase_s,FractionalRelease,Mass,Cg,Iyy, ...
        'VariableNames',{'t_s','cfg','pos_n_truth_m','pos_n_est_m','h_truth_m','h_est_m','Va_truth_mps','Va_est_mps','gamma_truth_rad','gamma_est_rad','theta_truth_rad','theta_est_rad','q_truth_radps','q_est_radps','N1_truth','N1_est','N2_truth','N2_est','Vg_truth_mps','Vg_est_mps','Vz_truth_mps','Vz_est_mps','wind_truth_mps','wind_raw_mps','wind_est_mps','wind_rate_est_mps2','wind_sigma_mps','wind_confidence','wind_rate_confidence','wind_effective_mps','wind_rate_effective_mps2','guide_vg_mps','guide_va_mps','guide_vz_mps','forecast_dwind1_mps','residual_observer_norm','wind_mpc_active','recovery_severity','recovery_level','gust_recovery_latched','abrupt_gust_trigger','release_wind_evidence_active','carrier_transient_evidence_active','disturbance_evidence_active','energy_href_shift_m','input_snap_magnitude','current_target_m','predicted_impact_m','drop_event','release_index','elevator_cmd','throttle_cmd','qp_exitflag','qp_time_s','prediction_error_norm_inf','primary_normalized_truth','causal_score_mask','release_phase_s','fractional_release','mass_truth_kg','cg_x_truth_m','Iyy_truth_kgm2'});
    ...
    report=struct("version","Physics-MPC v1.3.6-Paper fair-sensor transient-evidence energy-aware precision airdrop","pass",paper_core.pass,"paper_core",paper_core,"engineering_pass",engineering_gate.pass,"engineering_gate",engineering_gate,"scenario",opts.ScenarioName,"mode",mode,"control_mode",controlMode,"release_mode",releaseMode,"oracle_version",vOracle,"options",opts,"metrics",metrics,"gate",engineering_gate,"cargo",Cargo,"gust_event_indices",gustEvents,"completed_at",datetime("now"));
...
fprintf(fid,"Physics-MPC v1.3.6-Paper fair-sensor transient-evidence energy-aware precision airdrop\nscenario=%s\nmode=%s\npaper_core_pass=%d\nengineering_gate_pass=%d\n",r.scenario,r.mode,r.pass,r.engineering_pass);
fprintf(fid,"jsbsim_state_truth_used_by_controller=0\njsbsim_navigation_truth_used_by_release=0\ntruth_wind_used_by_controller=0\ntruth_wind_used_by_release_guidance=0\ncarrier_state_source=unbiased_noisy_baro_pitot_GNSS_INS_AHRS_IMU_engine_telemetry\nrelease_position_source=unbiased_noisy_GNSS_INS_estimate\ncarrier_truth=persistent_nonlinear_JSBSim_scoring_only\n");
if r.metrics.independent_cargo_truth_enabled, fprintf(fid,"payload_truth=independent_2D_vector_drag_sensitivity_model\n"); else, fprintf(fid,"payload_truth=nominal_ballistic_scoring_model\n"); end
...
pn=fieldnames(r.paper_core); for i=1:numel(pn), fprintf(fid,"paper_%s=%d\n",pn{i},r.paper_core.(pn{i})); end
gn=fieldnames(r.engineering_gate); for i=1:numel(gn), fprintf(fid,"engineering_%s=%d\n",gn{i},r.engineering_gate.(gn{i})); end
end
function localPlot(T,C,out,scenario,mode,gustEvents)
try
    f=figure("Visible","off","Position",[80 80 1550 980]); tl=tiledlayout(f,2,2,"TileSpacing","compact","Padding","compact"); title(tl,"AirdropX v1.3.6-Paper "+scenario+" / "+mode,"Interpreter","none");
    nexttile; plot(T.t_s,T.wind_truth_mps,"k-",T.t_s,T.wind_est_mps,"-"); grid on; ylabel("wind (m/s)"); xlabel("t (s)"); legend("truth","estimate");
    nexttile; plot(T.t_s,T.primary_normalized_truth); yline(1,'--'); grid on; ylabel("primary normalized"); xlabel("t (s)"); hold on; for j=gustEvents(:).', xline(T.t_s(j),':'); end
