...
$root=E $ProjectRoot; $cal=Join-Path $ProjectRoot 'matlab\results\physics_mpc_v130_wind_disturbance_calibration\wind_disturbance_model_v130.mat'; if(-not(Test-Path $cal)){throw 'Run full v1.3.4 runner once to create wind disturbance calibration.'}
$out=Join-Path $ProjectRoot ("matlab\results\physics_mpc_v134_stable_energy_recovery_point\${Scenario}_${Mode}"); New-Item -ItemType Directory -Force -Path $out|Out-Null
