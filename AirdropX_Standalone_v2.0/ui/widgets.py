...
class TrajectoryView(QWidget):
    ...
    def paintEvent(self,_):
        ...
        if self.current:
            ...
            lines=[f't = {self.current.t:5.1f} s    cfg = {self.current.cfg}',f'x = {self.current.x_m:7.1f} m   H = {self.current.h_m:6.2f} m',f'Va = {self.current.va_mps:5.2f} m/s  Pitch = {self.current.pitch_deg:+5.2f}°',f'Wind = {self.current.wind_mps:+5.2f} / est {self.current.wind_est_mps:+5.2f} m/s']
...
class MiniChart(QWidget):
    def __init__(self,title,parent=None): super().__init__(parent); self.title=title; self.data=[]; self.setMinimumHeight(145)
