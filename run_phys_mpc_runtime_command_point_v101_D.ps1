param(
  [ValidateSet('altitude_down_v45','altitude_up_v65','speed_up_h20','speed_down_h200','coupled_low_to_high','coupled_high_to_low')][string]$Scenario='altitude_down_v45',
...
$bank=Join-Path $ProjectRoot 'matlab\results\physics_mpc_v082_fixed_horizon_envelope_bank\physics_full_envelope_bank_diagnostic.mat'; if(-not (Test-Path $bank)){throw "Master bank missing: $bank"}; $out=Join-Path $ProjectRoot ("matlab\results\physics_mpc_v101_runtime_command_point\"+$Scenario+'_'+$PreviewMode); New-Item -ItemType Directory -Force -Path $out | Out-Null
$r=Escape-MatlabString $ProjectRoot; $b=Escape-MatlabString $bank; $o=Escape-MatlabString $out; $cmd="cd('$r'); addpath('matlab'); addpath('matlab/phys_mpc'); addpath('matlab/sfunc_jsbsim'); x=airdropx_phys_runtime_entry_v101(string(pwd),OutputRoot=string('$o'),ScenarioName=string('$Scenario'),MasterBankPath=string('$b'),Duration_s=150,CommandPreviewMode=string('$PreviewMode')); disp(x);"; & $MatlabExe -batch $cmd
