...
$out=Join-Path $ProjectRoot 'matlab\results\physics_mpc_v101_dynamic_reference_validation'; New-Item -ItemType Directory -Force -Path $out | Out-Null
$interval=Join-Path $ProjectRoot 'matlab\results\physics_mpc_v090_continuous_interval_validation'; $baseline=Join-Path $ProjectRoot 'matlab\results\physics_mpc_v100_runtime_command_validation'
$rootEsc=Escape-MatlabString $ProjectRoot; $masterEsc=Escape-MatlabString $master; $outEsc=Escape-MatlabString $out; $intervalEsc=Escape-MatlabString $interval; $baselineEsc=Escape-MatlabString $baseline
$manifest=Join-Path $out 'runtime_command_manifest.csv'; $manifestEsc=Escape-MatlabString $manifest
$preflight=Join-Path $out 'reference_selftest_terminal.txt'; $cmdP="cd('$rootEsc'); addpath('matlab'); addpath('matlab/phys_mpc'); r=airdropx_phys_runtime_reference_selftest_v101(string('$masterEsc')); disp(r);"; & $MatlabExe -batch $cmdP 2>&1 | Tee-Object -FilePath $preflight; if($LASTEXITCODE -ne 0){throw 'v1.0.1 dynamic-reference preflight failed.'}
$cmdM="cd('$rootEsc'); addpath('matlab'); addpath('matlab/phys_mpc'); T=airdropx_phys_runtime_scenario_manifest_v101(string('$manifestEsc')); disp(T);"; & $MatlabExe -batch $cmdM 2>&1 | Tee-Object -FilePath (Join-Path $out 'manifest_terminal.txt'); if($LASTEXITCODE -ne 0 -or -not (Test-Path $manifest)){throw 'Runtime v1.0.1 scenario manifest failed.'}
...
while($queue.Count -gt 0 -or $active.Count -gt 0){
  while($queue.Count -gt 0 -and $active.Count -lt $MaxParallel){
    ...
    if((-not $Force) -and (Test-Path $marker) -and (Test-Path $mat)){Write-Host "[$tag] reusing v1.0.1 complete result"; $records += [pscustomobject]@{Scenario=$tag;PID=0;Elapsed_s=0;ForcedExitAfterResult=$false;Reused=$true;InfraFail=$false;Timeout=$false}; continue}
    ...
    $cmd="cd('$rootEsc'); addpath('matlab'); addpath('matlab/phys_mpc'); addpath('matlab/sfunc_jsbsim'); r=airdropx_phys_runtime_entry_v101(string(pwd),OutputRoot=string('$dirEsc'),ScenarioName=string('$tag'),MasterBankPath=string('$masterEsc'),Duration_s=$duration,CommandPreviewMode=string('$mode')); disp(r);"
...
$cmdF="cd('$rootEsc'); addpath('matlab'); addpath('matlab/phys_mpc'); r=airdropx_phys_finalize_runtime_v101(string(pwd),OutputRoot=string('$outEsc'),ManifestPath=string('$manifestEsc'),IntervalEvidenceRoot=string('$intervalEsc'),BaselineV100Root=string('$baselineEsc')); disp(r);"; $final=Join-Path $out 'finalize_terminal.txt'; & $MatlabExe -batch $cmdF 2>&1 | Tee-Object -FilePath $final; if($LASTEXITCODE -ne 0){throw "Runtime v1.0.1 finalizer failed. See $final"}
$summary=Join-Path $out 'runtime_command_validation_summary.txt'; if(-not (Test-Path $summary)){throw 'Missing runtime v1.0.1 summary.'}; Write-Host '=== Physics-MPC v1.0.1 DYNAMIC-FEASIBLE RUNTIME FLOW COMPLETE ==='; Get-Content $summary
