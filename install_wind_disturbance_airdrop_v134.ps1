...
$stamp=Get-Date -Format 'yyyyMMdd_HHmmss'; $backup=Join-Path $ProjectRoot ("matlab\_backup_v134_"+$stamp)
...
CopySafe (Join-Path $Here 'run_wind_disturbance_airdrop_v134_D.ps1') (Join-Path $ProjectRoot 'run_wind_disturbance_airdrop_v134_D.ps1')
CopySafe (Join-Path $Here 'run_wind_disturbance_airdrop_point_v134_D.ps1') (Join-Path $ProjectRoot 'run_wind_disturbance_airdrop_point_v134_D.ps1')
Write-Host "Installed Physics-MPC v1.3.4 into $ProjectRoot"
...
Write-Host 'v1.3.4 adds strict abrupt-event recovery, calm-exact legacy fallback, bounded altitude-scaled energy exchange, bias-cancelled Iyy delta validation, and quiet plotting.'
Write-Host 'Run: .\run_wind_disturbance_airdrop_v134_D.ps1'
