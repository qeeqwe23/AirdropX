...
class MonitorPanel(QWidget):
    def __init__(self, parent=None):
        ...
        lay.addWidget(title)
        self.banner = QLabel("Physics-MPC v1.4.0 · 等待任务")
    ...
    def reset(self):
        for c in [self.height_chart, self.pitch_chart, self.control_chart, self.mass_chart, self.trajectory]:
            c.clear()
        self.banner.setText("Physics-MPC v1.4.0 · 等待任务")
    ...
    def show_result(self, res, cfg):
        ...
        self.banner.setText(f"完成 · H={cfg.target_altitude_m:g}m · V={cfg.target_speed_mps:g}m/s · {len(res.cargo)} cargo records")
