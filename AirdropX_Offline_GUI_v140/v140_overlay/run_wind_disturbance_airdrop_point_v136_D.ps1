...
if(-not(Test-Path $cal)){throw 'Run the full v1.3.6 runner once to create wind disturbance calibration.'}
...
# v1.3.6 does not change the validated v1.3.5 mass-refresh Oracle. Reuse it;
...
$out=Join-Path $ProjectRoot ("matlab\results\physics_mpc_v136_transient_energy_recovery_point\${Scenario}_${Mode}")
...
$cmd="cd('$root'); addpath('matlab'); addpath('matlab/phys_mpc'); addpath('matlab/wind'); addpath('matlab/airdrop'); r=airdropx_wind_airdrop_entry_v136(string(pwd),OutputRoot=string('$outEsc'),ScenarioName=string('$Scenario'),UseWindCompensation=$aware,UseWindDisturbanceMPC=$wm,UseFractionalRelease=$frac,UseWindConfidenceGate=$conf,UseUnifiedGustRecovery=$rec,WindCalibrationPath=string('$calEsc'),Duration_s=$duration,SensorNoiseSeed=$Seed); disp(r);"
