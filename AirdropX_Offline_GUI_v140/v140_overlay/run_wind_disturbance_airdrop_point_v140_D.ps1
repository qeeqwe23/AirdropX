...
if(-not(Test-Path $cal)){throw 'Run the full v1.4.0 runner once, or provide the existing v1.3.0 wind-disturbance calibration.'}
...
# v1.4.0 reuses the validated v1.3.5 mass-refresh Oracle. Reuse it;
# only rebuild if the mex/marker is actually missing.
...
if((-not(Test-Path $mex)) -or (-not(Test-Path $oracleMarker))){
...
if($SharedCargoTruth){$variant+='_sharedCargo'}
$out=Join-Path $ProjectRoot ("matlab\results\physics_mpc_v140_sensor_realistic_point\${Scenario}_${Mode}_${variant}")
New-Item -ItemType Directory -Force -Path $out|Out-Null
$aware=if($Mode -eq 'legacy_mpc_nowind'){'false'}else{'true'}; $wm=if($Mode -eq 'wind_mpc_aware'){'true'}else{'false'}; $frac=if($SampledRelease){'false'}else{'true'}; $rec=if($NoRecovery){'false'}else{'true'}; $conf=if($NoConfidence){'false'}else{'true'}; $outEsc=E $out; $calEsc=E $cal; $realAv=if($IdealStateFeedback){'false'}else{'true'}; $indCargo=if($SharedCargoTruth){'false'}else{'true'}
$cmd="cd('$root'); addpath('matlab'); addpath('matlab/phys_mpc'); addpath('matlab/wind'); addpath('matlab/airdrop'); addpath('matlab/avionics'); r=airdropx_wind_airdrop_entry_v140(string(pwd),OutputRoot=string('$outEsc'),ScenarioName=string('$Scenario'),UseWindCompensation=$aware,UseWindDisturbanceMPC=$wm,UseFractionalRelease=$frac,UseWindConfidenceGate=$conf,UseUnifiedGustRecovery=$rec,UseRealisticAvionics=$realAv,UseIndependentCargoTruth=$indCargo,WindCalibrationPath=string('$calEsc'),Duration_s=$duration,SensorNoiseSeed=$Seed); disp(r);"
