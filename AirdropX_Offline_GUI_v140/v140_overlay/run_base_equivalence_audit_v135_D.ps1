...
$out=Join-Path $ProjectRoot 'matlab\results\physics_mpc_v135_base_equivalence_audit'; New-Item -ItemType Directory -Force -Path $out|Out-Null
...
if((-not(Test-Path $mex)) -or (-not(Test-Path $marker))){
  $so=Join-Path $out 'oracle_build_stdout.txt';$se=Join-Path $out 'oracle_build_stderr.txt';$cmd="cd('$root'); addpath('matlab'); addpath('matlab/phys_mpc'); build_airdropx_jsbsim_wind_oracle_v121;";$p=StartMatlab $cmd $so $se;if($p.ExitCode -ne 0){if(Test-Path $so){Get-Content $so};if(Test-Path $se){Get-Content $se};throw 'v1.3.5 Oracle build failed.'}
}
if(-not(Test-Path $marker)){
  $so=Join-Path $out 'oracle_selftest_stdout.txt';$se=Join-Path $out 'oracle_selftest_stderr.txt';$cmd="cd('$root'); addpath('matlab'); addpath('matlab/phys_mpc'); r=airdropx_phys_wind_oracle_selftest_v121(string(pwd)); assert(contains(string(r.version),'mass-refresh-v135')); disp(r);";$p=StartMatlab $cmd $so $se;if($p.ExitCode -ne 0){if(Test-Path $so){Get-Content $so};if(Test-Path $se){Get-Content $se};throw 'v1.3.5 Oracle selftest failed.'};Set-Content -Encoding ASCII -Path $marker -Value ("pass "+(Get-Date -Format o))
}
$cmd="cd('$root'); addpath('matlab'); addpath('matlab/phys_mpc'); addpath('matlab/wind'); addpath('matlab/airdrop'); R=airdropx_phys_mpc_base_equivalence_audit_v135(string(pwd),OutputRoot=string('$(E $out)'),WindCalibrationPath=string('$(E $cal)'),SensorNoiseSeed=$Seed); disp(R); if ~R.pass, error('AirdropX:WindMPC:BaseEquivalenceFailed','Base equivalence audit failed.'); end"
