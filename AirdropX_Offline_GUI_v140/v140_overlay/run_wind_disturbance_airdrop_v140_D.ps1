...
$out=Join-Path $ProjectRoot 'matlab\results\physics_mpc_v140_sensor_realistic_airdrop_validation'; New-Item -ItemType Directory -Force -Path $out|Out-Null
...
# Stage 2. Validate both onboard predictor calibration and the independent cargo scoring plant.
$manifest=Join-Path $out 'wind_airdrop_manifest.csv'; $manifestEsc=E $manifest
$cmd="cd('$rootEsc'); addpath('matlab'); addpath('matlab/wind'); addpath('matlab/airdrop'); addpath('matlab/avionics'); p=airdropx_airdrop_ballistic_params_v121; q=airdropx_cargo_truth_params_v140; assert(abs(p.calibration_error_m)<1e-8); assert(abs(q.calibration_error_m)<1e-6); assert(contains(string(q.version),'independent cargo truth plant')); T=airdropx_wind_airdrop_manifest_v136(string('$manifestEsc')); disp(p); disp(q); disp(T);"
$code=Invoke-MatlabCapture $cmd (Join-Path $out 'manifest_terminal.txt'); if($code -ne 0 -or -not(Test-Path $manifest)){throw 'Predictor/independent-cargo calibration or manifest preflight failed.'}
...
$eqRoot=Join-Path $ProjectRoot 'matlab\results\physics_mpc_v140_sensor_base_equivalence_audit'; $eqEsc=E $eqRoot
$cmd="cd('$rootEsc'); addpath('matlab'); addpath('matlab/phys_mpc'); addpath('matlab/wind'); addpath('matlab/airdrop'); addpath('matlab/avionics'); R=airdropx_phys_mpc_base_equivalence_audit_v140(string(pwd),OutputRoot=string('$eqEsc'),WindCalibrationPath=string('$calEsc'),SensorNoiseSeed=101); disp(R); assert(R.pass);"
$code=Invoke-MatlabCapture $cmd (Join-Path $out 'base_equivalence_terminal.txt'); if($code -ne 0){throw 'v1.4.0 sensor-realistic base-equivalence audit failed. Full suite intentionally not started.'}
$jobs=@()
...
function StartCase($j){
  ...
  $cmd="cd('$rootEsc'); addpath('matlab'); addpath('matlab/phys_mpc'); addpath('matlab/wind'); addpath('matlab/airdrop'); addpath('matlab/avionics'); r=airdropx_wind_airdrop_entry_v140(string(pwd),OutputRoot=string('$dirEsc'),ScenarioName=string('$($j.Scenario)'),UseWindCompensation=$aware,UseWindDisturbanceMPC=$wm,UseFractionalRelease=$frac,UseWindConfidenceGate=$conf,UseUnifiedGustRecovery=$rec,UseRealisticAvionics=true,UseIndependentCargoTruth=true,WindCalibrationPath=string('$calEsc'),Duration_s=$($j.Duration),TargetStart_m=$($j.TargetStart),TargetSpacing_m=$($j.Spacing),SensorNoiseSeed=$($j.Seed)); disp(r);"
...
$cmd="cd('$rootEsc'); addpath('matlab'); addpath('matlab/airdrop'); addpath('matlab/avionics'); R=airdropx_wind_airdrop_finalize_v140(string('$outEsc'),string('$manifestEsc')); disp(R);"
$code=Invoke-MatlabCapture $cmd (Join-Path $out 'finalize_terminal.txt'); if($code -ne 0){throw 'v1.4.0 sensor-realistic finalizer failed.'}
Get-Content (Join-Path $out 'wind_sensor_realism_validation_summary.txt')
