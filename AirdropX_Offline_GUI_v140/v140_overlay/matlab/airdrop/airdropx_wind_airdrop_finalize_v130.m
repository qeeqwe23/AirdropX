function R=airdropx_wind_airdrop_finalize_v130(root,manifestPath)
%AIRDROPX_WIND_AIRDROP_FINALIZE_V130 Aggregate 3-way carrier/release comparisons.
arguments
    root (1,1) string
...
for i=1:height(M)
    for mode=modes
        ...
        found=isfile(p); pass=false; drops=0; maxLand=NaN; rmsLand=NaN; windP95=NaN; instPeak=NaN; causalPeak=NaN; rec=NaN; postRms=NaN; qp=NaN; pred=NaN;
        if found
        ...
        one=table(M.Scenario(i),mode,found,pass,drops,maxLand,rmsLand,windP95,instPeak,causalPeak,rec,postRms,qp,pred, ...
            'VariableNames',{'Scenario','Mode','Found','Pass','Drops','MaxLandingError_m','RmsLandingError_m','WindErrorP95_mps','InstantPeakNorm','PostGustPeakNorm','MaxRecovery_s','PostWindRmsNorm','QpP95_ms','PredictionP95'});
...
R=struct(); R.version="Physics-MPC v1.3.0 wind-disturbance-aware precision airdrop validation"; R.pass=passIntegrated && allExecuted; R.integrated_pass=passIntegrated; R.all_three_way_executed=allExecuted; R.rows=rows; R.comparison=comparison; R.median_legacy_over_wind_mpc_carrier_rms=medianCarrierRatio; R.completed_at=datetime("now");
