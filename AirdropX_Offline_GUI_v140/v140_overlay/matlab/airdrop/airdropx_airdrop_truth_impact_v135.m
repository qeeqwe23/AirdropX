function out=airdropx_airdrop_truth_impact_v135(scenarioName,release,opts)
%AIRDROPX_AIRDROP_TRUTH_IMPACT_V135 Scoring-only cargo truth using the v1.3.5 validation wind profile.
arguments
...
windFun=@(tau) double(airdropx_wind_profile_v135(scenarioName,double(release.t_s)+tau,SineForcingEnd_s=opts.SineForcingEnd_s,SineSettleRamp_s=opts.SineSettleRamp_s));
out=localIntegrate(double(release.x_m),double(release.h_m),double(release.vx_ground_mps),double(release.vz_up_mps),windFun,p);
out.mode="scoring_only_true_wind_profile_v135";
end
