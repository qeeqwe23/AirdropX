function report=airdropx_wind_airdrop_mission_v140(projectRoot,opts)
%AIRDROPX_WIND_AIRDROP_MISSION_V140 Sensor-realistic onboard MPC + transient-evidence gust recovery.
%
% Plant truth: persistent nonlinear JSBSim wind Oracle.
% Onboard state/control: sensor-realistic avionics layer (GNSS/INS, baro, pitot, IMU/AHRS, engine telemetry).
...
arguments
    ...
    opts.SigmaGnssPosition_m (1,1) double {mustBeNonnegative} = 0.60
    opts.SigmaBaroAltitude_m (1,1) double {mustBeNonnegative} = 0.30
    opts.SigmaPitch_deg (1,1) double {mustBeNonnegative} = 0.08
    opts.SigmaPitchRate_dps (1,1) double {mustBeNonnegative} = 0.02
    opts.SigmaEngineN1 (1,1) double {mustBeNonnegative} = 0.08
    opts.SigmaEngineN2 (1,1) double {mustBeNonnegative} = 0.08
...
if opts.OutputRoot=="", opts.OutputRoot=fullfile(projectRoot,"matlab","results","physics_mpc_v140_sensor_realistic_airdrop",opts.ScenarioName+"_"+mode); end
...
try
    ...
    avionics=airdropx_avionics_init_v140(Ts,SigmaGnssPos_m=opts.SigmaGnssPosition_m,SigmaGnssGroundspeed_mps=opts.SigmaGroundspeed_mps, ...
    ...
    for k=1:Nsim
        ...
        if opts.UseRealisticAvionics
            [avionics,av]=airdropx_avionics_step_v140(avionics,sensorTruth);
            ...
            % hardware-realistic performance.
        ...
        % v1.4.0 retains the v1.3.6 split between two meanings of "wind evidence":
        ...
        % Release guidance. Truth wind is prohibited here. v1.4.0 keeps the
        ...
        if cfg<4
            ...
            if SR.release_now
                ...
                if opts.UseIndependentCargoTruth
                    truth=airdropx_cargo_truth_plant_v140(opts.ScenarioName,truthRelease,SineForcingEnd_s=opts.SineForcingEnd_s,SineSettleRamp_s=opts.SineSettleRamp_s,CdAScale=opts.CargoTruthCdAScale);
                else
                ...
                fprintf("[V140 DROP] %s %s cargo%d t=%.3f phase=0 target=%.2f release=%.2f pred=%.2f truth=%.2f miss=%+.2f windEst=%+.2f truthWind=%+.2f\n", ...
        ...
        if opts.UseWindDisturbanceMPC
            % v1.4.0: strong recovery remains event-gated. A real abrupt gust must
        ...
        if scheduledRelease
            ...
            fprintf("[V140 DROP] %s %s cargo%d t=%.3f phase=%.3f target=%.2f release=%.2f pred=%.2f truth=%.2f miss=%+.2f windEst=%+.2f truthWind=%+.2f\n", ...
        ...
        if k==1 || DropEvent(k) || mod(k,50)==0
            fprintf("[V140] %s %s t=%5.1f cfg=%d x=%.1f hErr=%+.3f VaErr=%+.3f wind=%+.2f eff=%+.2f truth=%+.2f conf=%.2f rec=%.2f dW1=%+.3f res=%.3g qp=%.2fms\n", ...
    ...
    metrics.realistic_avionics_enabled=opts.UseRealisticAvionics; metrics.independent_cargo_truth_enabled=opts.UseIndependentCargoTruth;
    ...
    cp=airdropx_cargo_truth_params_v140(); metrics.independent_cargo_truth_CdA_m2=cp.CdA_m2; metrics.independent_cargo_truth_calibration_error_m=cp.calibration_error_m;

    gate=struct(); gate.realistic_avionics=opts.UseRealisticAvionics; gate.independent_cargo_truth=opts.UseIndependentCargoTruth; gate.truth_not_used_for_release=1; gate.truth_not_used_by_controller=1; gate.four_drops=metrics.drops_completed==4; gate.wind_estimation=metrics.wind_error_p95_mps<=opts.MaxWindErrorP95_mps;
    ...
    report=struct("version","Physics-MPC v1.4.0 sensor-realistic transient-evidence energy-aware precision airdrop","pass",gate.pass,"scenario",opts.ScenarioName,"mode",mode,"control_mode",controlMode,"release_mode",releaseMode,"oracle_version",vOracle,"options",opts,"metrics",metrics,"gate",gate,"cargo",Cargo,"gust_event_indices",gustEvents,"completed_at",datetime("now"));
...
fprintf(fid,"Physics-MPC v1.4.0 sensor-realistic transient-evidence energy-aware precision airdrop\nscenario=%s\nmode=%s\npass=%d\n",r.scenario,r.mode,r.pass);
fprintf(fid,"jsbsim_state_truth_used_by_controller=0\njsbsim_navigation_truth_used_by_release=0\ntruth_wind_used_by_controller=0\ntruth_wind_used_by_release_guidance=0\ncarrier_state_source=baro_pitot_GNSS_INS_AHRS_IMU_engine_telemetry\nrelease_position_source=GNSS_INS_estimate\ncarrier_truth=persistent_nonlinear_JSBSim_scoring_only\npayload_truth=independent_2D_vector_drag_cargo_plant_scoring_only\n");
...
try
    f=figure("Visible","off","Position",[80 80 1550 980]); tl=tiledlayout(f,2,2,"TileSpacing","compact","Padding","compact"); title(tl,"AirdropX v1.4.0 "+scenario+" / "+mode,"Interpreter","none");
