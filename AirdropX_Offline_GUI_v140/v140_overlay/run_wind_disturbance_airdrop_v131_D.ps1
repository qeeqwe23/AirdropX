...
$out=Join-Path $ProjectRoot 'matlab\results\physics_mpc_v131_wind_disturbance_airdrop_validation'; New-Item -ItemType Directory -Force -Path $out|Out-Null
...
foreach($j in $profiles){
  $jobs += [pscustomobject]@{Scenario=$j.Scenario;Mode='wind_mpc_aware';WindMpc=$true;Aware=$true;Fractional=$true;Duration=[double]$j.Duration_s;TargetStart=[double]$j.TargetStart_m;Spacing=[double]$j.TargetSpacing_m;Seed=[int]$j.SensorNoiseSeed}
  $jobs += [pscustomobject]@{Scenario=$j.Scenario;Mode='legacy_mpc_aware';WindMpc=$false;Aware=$true;Fractional=$true;Duration=[double]$j.Duration_s;TargetStart=[double]$j.TargetStart_m;Spacing=[double]$j.TargetSpacing_m;Seed=[int]$j.SensorNoiseSeed}
  $jobs += [pscustomobject]@{Scenario=$j.Scenario;Mode='legacy_mpc_nowind';WindMpc=$false;Aware=$false;Fractional=$true;Duration=[double]$j.Duration_s;TargetStart=[double]$j.TargetStart_m;Spacing=[double]$j.TargetSpacing_m;Seed=[int]$j.SensorNoiseSeed}
}
# One diagnostic calm ablation keeps the old sample-boundary release timing.
# It is not a formal gate; it measures how much of the calm miss came from the
# 0.1 s / ~5 m release-position quantization.
$calmRow=$profiles|Where-Object {$_.Scenario -eq 'calm'}|Select-Object -First 1
if($null -ne $calmRow){$jobs += [pscustomobject]@{Scenario='calm';Mode='wind_mpc_aware_sampled_release';WindMpc=$true;Aware=$true;Fractional=$false;Duration=[double]$calmRow.Duration_s;TargetStart=[double]$calmRow.TargetStart_m;Spacing=[double]$calmRow.TargetSpacing_m;Seed=[int]$calmRow.SensorNoiseSeed}}
$records=@()
function StartCase($j){
  ...
  $aware=if($j.Aware){'true'}else{'false'}; $wm=if($j.WindMpc){'true'}else{'false'}; $frac=if($j.Fractional){'true'}else{'false'}
  $cmd="cd('$rootEsc'); addpath('matlab'); addpath('matlab/phys_mpc'); addpath('matlab/wind'); addpath('matlab/airdrop'); r=airdropx_wind_airdrop_entry_v131(string(pwd),OutputRoot=string('$dirEsc'),ScenarioName=string('$($j.Scenario)'),UseWindCompensation=$aware,UseWindDisturbanceMPC=$wm,UseFractionalRelease=$frac,WindCalibrationPath=string('$calEsc'),Duration_s=$($j.Duration),TargetStart_m=$($j.TargetStart),TargetSpacing_m=$($j.Spacing),SensorNoiseSeed=$($j.Seed)); disp(r);"
...
# do not launch the remaining 24 missions (23 three-way cases + 1 calm timing ablation).
...
$cmd="cd('$rootEsc'); addpath('matlab'); addpath('matlab/airdrop'); R=airdropx_wind_airdrop_finalize_v131(string('$outEsc'),string('$manifestEsc')); disp(R);"
