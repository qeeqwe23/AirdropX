...
$stamp=Get-Date -Format 'yyyyMMdd_HHmmss'; $backup=Join-Path $ProjectRoot ("matlab\_backup_v131_"+$stamp)
...
Copy-Item (Join-Path $Here 'run_wind_disturbance_airdrop_v131_D.ps1') (Join-Path $ProjectRoot 'run_wind_disturbance_airdrop_v131_D.ps1') -Force
Copy-Item (Join-Path $Here 'run_wind_disturbance_airdrop_point_v131_D.ps1') (Join-Path $ProjectRoot 'run_wind_disturbance_airdrop_point_v131_D.ps1') -Force
Write-Host "Installed Physics-MPC v1.3.1 wind-disturbance-aware airdrop into $ProjectRoot"
...
Write-Host 'Existing v0.3.3 Oracle MEX is untouched; v1.3.1 reuses the separate v1.2.1 continuous-wind Oracle.'
Write-Host 'Run: .\run_wind_disturbance_airdrop_v131_D.ps1'
