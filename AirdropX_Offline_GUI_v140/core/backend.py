...
class MatlabBackend:
    ...
    def preflight(self, cfg: MissionConfig) -> tuple[BackendPaths | None, list[str]]:
        ...
        required = [
            ...
            project_root / "matlab" / "wind" / "airdropx_wind_profile_v140_gui.m",
        ]
        for p in required:
            if not p.exists():
                issues.append(f"缺少集成文件: {p}")
        if cfg.model_policy == "auto_cache":
            p = project_root / "matlab" / "phys_mpc" / "airdropx_phys_build_bank.m"
            if not p.exists():
                issues.append(f"自由 H/V 模式缺少建模函数: {p}")
    ...
    def run(
        ...
        output_root = Path(cfg.output_root) if cfg.output_root else paths.project_root / "matlab" / "results" / "offline_gui_v140" / stamp
        ...
        mcmd = (
            ...
            f"airdropx_gui_backend_entry_v140('{matlab_quote(paths.project_root)}','{matlab_quote(config_path)}');"
        )
