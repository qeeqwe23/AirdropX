...
$out=Join-Path $ProjectRoot 'matlab\results\physics_mpc_v133_energy_recovery_airdrop_validation'; New-Item -ItemType Directory -Force -Path $out|Out-Null
...
$cmd="cd('$rootEsc'); addpath('matlab'); addpath('matlab/airdrop'); R=airdropx_wind_airdrop_finalize_v133(string('$outEsc'),string('$manifestEsc')); disp(R);"
$code=Invoke-MatlabCapture $cmd (Join-Path $out 'finalize_terminal.txt'); if($code -ne 0){throw 'v1.3.3 energy-recovery finalizer failed.'}
Get-Content (Join-Path $out 'wind_energy_recovery_validation_summary.txt')
