...
$out=Join-Path $ProjectRoot 'matlab\results\physics_mpc_v136_base_equivalence_audit'; New-Item -ItemType Directory -Force -Path $out|Out-Null
...
$cmd="cd('$root'); addpath('matlab'); addpath('matlab/phys_mpc'); addpath('matlab/wind'); addpath('matlab/airdrop'); R=airdropx_phys_mpc_base_equivalence_audit_v136(string(pwd),OutputRoot=string('$(E $out)'),WindCalibrationPath=string('$(E $cal)'),SensorNoiseSeed=$Seed); disp(R); if ~R.pass, error('AirdropX:WindMPC:BaseEquivalenceFailed','Base equivalence audit failed.'); end"
