function R=airdropx_wind_airdrop_finalize_v132(root,manifestPath)
%AIRDROPX_WIND_AIRDROP_FINALIZE_V132 Aggregate v1.3.2 3-way comparisons and recovery diagnostics.
arguments
    root (1,1) string
...
for i=1:height(M)
    for mode=modes
        ...
        one=table(M.Scenario(i),mode,found,pass,drops,maxLand,rmsLand,windP95,instPeak,causalPeak,rec,postRms,tail5,finalNorm,r05,r10,r30,qp,pred,fracCount,schedResidual,windActive,recoveryActive,recoveryPeak,elevSat,thrSat,elevHead,thrHead,confMean, ...
            'VariableNames',{'Scenario','Mode','Found','Pass','Drops','MaxLandingError_m','RmsLandingError_m','WindErrorP95_mps','InstantPeakNorm','PostGustPeakNorm','MaxRecovery_s','PostWindRmsNorm','Tail5RmsNorm','FinalNormInf','Residual0p5Norm','Residual1p0Norm','Residual3p0Norm','QpP95_ms','PredictionP95','FractionalReleaseCount','SchedulerResidualMax_m','WindMpcActiveFraction','RecoveryActiveFraction','RecoveryLevelPeak','ElevatorSatFraction','ThrottleSatFraction','ElevatorMinHeadroom','ThrottleMinHeadroom','WindConfidenceMean'});
...
R=struct(); R.version="Physics-MPC v1.3.2 confidence-gated gust-recovery precision airdrop validation"; R.pass=passIntegrated && allExecuted; R.integrated_pass=passIntegrated; R.all_three_way_executed=allExecuted; R.rows=rows; R.comparison=comparison; R.median_legacy_over_wind_mpc_carrier_rms=medianCarrierRatio; R.completed_at=datetime("now");
...
writetable(rows,fullfile(root,"wind_recovery_validation.csv")); writetable(comparison,fullfile(root,"wind_recovery_comparison.csv")); save(fullfile(root,"wind_recovery_validation.mat"),"R","-v7.3");
localWrite(R,fullfile(root,"wind_recovery_validation_summary.txt")); localPlot(comparison,root);
end
function localWrite(R,path)
fid=fopen(path,"w"); if fid<0, return; end; c=onCleanup(@()fclose(fid)); %#ok<NASGU>
fprintf(fid,"Physics-MPC v1.3.2 confidence-gated gust-recovery precision airdrop validation\nvalidation_pass=%d\nintegrated_wind_mpc_scenarios_pass=%d/8\nall_three_way_missions_found=%d/24\n",R.pass,sum(R.rows.Pass & R.rows.Mode=="wind_mpc_aware"),sum(R.rows.Found));
...
try
    ...
    nexttile; bar(categorical(C.Scenario),[C.WindMpcPostWindRms C.LegacyPostWindRms]); ylabel("post-wind primary RMS"); legend("v1.3.2 wind/recovery MPC","legacy MPC"); grid on;
    nexttile; bar(categorical(C.Scenario),[C.WindMpcRecovery_s C.LegacyRecovery_s]); ylabel("gust recovery (s)"); legend("v1.3.2","legacy"); grid on;
    ...
    exportgraphics(f,fullfile(root,"wind_recovery_validation.png"),"Resolution",160); close(f);
catch ME
