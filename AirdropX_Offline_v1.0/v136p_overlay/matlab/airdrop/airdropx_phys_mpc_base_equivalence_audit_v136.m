function R=airdropx_phys_mpc_base_equivalence_audit_v136(projectRoot,opts)
%AIRDROPX_PHYS_MPC_BASE_EQUIVALENCE_AUDIT_V136 Same-seed calm A/B between v1.3.6 wind path and certified legacy base solver.
arguments
...
if opts.OutputRoot=="", opts.OutputRoot=fullfile(projectRoot,"matlab","results","physics_mpc_v136_base_equivalence_audit"); end
...
rw=airdropx_wind_airdrop_mission_v136(projectRoot,OutputRoot=wmDir,ScenarioName="calm",UseWindCompensation=true,UseWindDisturbanceMPC=true,UseFractionalRelease=true,UseWindConfidenceGate=true,UseUnifiedGustRecovery=true,WindCalibrationPath=opts.WindCalibrationPath,Duration_s=opts.Duration_s,SensorNoiseSeed=opts.SensorNoiseSeed,ThrowOnFail=false);
rl=airdropx_wind_airdrop_mission_v136(projectRoot,OutputRoot=legacyDir,ScenarioName="calm",UseWindCompensation=true,UseWindDisturbanceMPC=false,UseFractionalRelease=true,UseWindConfidenceGate=true,UseUnifiedGustRecovery=false,WindCalibrationPath=opts.WindCalibrationPath,Duration_s=opts.Duration_s,SensorNoiseSeed=opts.SensorNoiseSeed,ThrowOnFail=false);
W=load(fullfile(wmDir,"wind_airdrop_mission.mat"),"T","Cargo"); L=load(fullfile(legacyDir,"wind_airdrop_mission.mat"),"T","Cargo");
TW=W.T; TL=L.T;
if height(TW)~=height(TL) || any(abs(TW.t_s-TL.t_s)>1e-14), error("AirdropX:WindMPC:EquivalenceTimebase","A/B time bases differ."); end
stateNames=["h_m","Va_mps","gamma_rad","theta_rad","q_radps","N1","N2","pos_n_m","Vg_long_mps","Vz_up_mps"];
...
if ~isempty(first)
    ...
    elseif maxStatePerRow(first)>opts.StateTolerance, firstReason="state";
...
Delta=table(TW.t_s,maxControlPerRow,maxStatePerRow,cfgMismatch,dropMismatch,phaseDiff,windPathUnexpected, ...
    'VariableNames',{'t_s','max_abs_control_delta','max_abs_state_delta','cfg_mismatch','release_event_mismatch','release_phase_delta_s','wind_path_unexpected'});
writetable(Delta,fullfile(opts.OutputRoot,"base_equivalence_delta.csv"));
R=struct(); R.version="Physics-MPC v1.3.6 base-equivalence audit"; R.sensor_noise_seed=opts.SensorNoiseSeed; R.wind_report_pass=rw.pass; R.legacy_report_pass=rl.pass;
...
save(fullfile(opts.OutputRoot,"base_equivalence_audit.mat"),"R","Delta","-v7.3");
localWrite(R,fullfile(opts.OutputRoot,"base_equivalence_summary.txt"));
end

function localWrite(R,path)
fid=fopen(path,"w"); if fid<0, return; end; c=onCleanup(@()fclose(fid)); %#ok<NASGU>
fprintf(fid,"Physics-MPC v1.3.6 base-equivalence audit\n");
