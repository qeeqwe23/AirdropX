function R=airdropx_wind_airdrop_finalize_v136p(root,manifestPath)
%AIRDROPX_WIND_AIRDROP_FINALIZE_V136P Aggregate paper-validation 3-way comparisons.
arguments
    root (1,1) string
...
for i=1:height(M)
    for mode=modes
        ...
        found=isfile(p); pass=false; engineeringPass=false; drops=0; maxLand=NaN; rmsLand=NaN; windP95=NaN; instPeak=NaN; causalPeak=NaN; rec=NaN; postRms=NaN; qp=NaN; pred=NaN; fracCount=NaN; schedResidual=NaN;
        ...
        stateEstP95=NaN; posEstP95=NaN; vgEstP95=NaN; vzEstP95=NaN; paperSensor=NaN; independentCargo=NaN;
        if found
            S=load(p,"report"); r=S.report; pass=logical(r.pass); if isfield(r,'engineering_pass'), engineeringPass=logical(r.engineering_pass); else, engineeringPass=logical(r.pass); end; drops=r.metrics.drops_completed; maxLand=r.metrics.max_landing_error_m; rmsLand=r.metrics.rms_landing_error_m; windP95=r.metrics.wind_error_p95_mps;
            ...
            if isfield(r.metrics,"onboard_state_estimation_p95_normalized"), stateEstP95=r.metrics.onboard_state_estimation_p95_normalized; end
            if isfield(r.metrics,"onboard_position_error_p95_m"), posEstP95=r.metrics.onboard_position_error_p95_m; end
            if isfield(r.metrics,"onboard_groundspeed_error_p95_mps"), vgEstP95=r.metrics.onboard_groundspeed_error_p95_mps; end
            if isfield(r.metrics,"onboard_vertical_speed_error_p95_mps"), vzEstP95=r.metrics.onboard_vertical_speed_error_p95_mps; end
            if isfield(r.metrics,"paper_sensor_model_enabled"), paperSensor=r.metrics.paper_sensor_model_enabled; end
            if isfield(r.metrics,"independent_cargo_truth_enabled"), independentCargo=r.metrics.independent_cargo_truth_enabled; end
        end
        one=table(M.Scenario(i),mode,found,pass,engineeringPass,drops,maxLand,rmsLand,windP95,instPeak,causalPeak,rec,postRms,tail5,finalNorm,r05,r10,r30,qp,pred,fracCount,schedResidual,windActive,recoveryActive,recoveryPeak,elevSat,thrSat,elevHead,thrHead,confMean,abruptCount,energyActive,energyShift,inputSnap,disturbanceEvidence,forcedRms,forcedPeak,releaseEvidence,carrierEvidence,carrierSwitch,windSwitch,lastCarrier,lastWindMpc,tailBase,tailZero,zeroSettle,stateEstP95,posEstP95,vgEstP95,vzEstP95,paperSensor,independentCargo, ...
            'VariableNames',{'Scenario','Mode','Found','PaperCorePass','EngineeringPass','Drops','MaxLandingError_m','RmsLandingError_m','WindErrorP95_mps','InstantPeakNorm','PostGustPeakNorm','MaxRecovery_s','PostWindRmsNorm','Tail5RmsNorm','FinalNormInf','Residual0p5Norm','Residual1p0Norm','Residual3p0Norm','QpP95_ms','PredictionP95','FractionalReleaseCount','SchedulerResidualMax_m','WindMpcActiveFraction','RecoveryActiveFraction','RecoveryLevelPeak','ElevatorSatFraction','ThrottleSatFraction','ElevatorMinHeadroom','ThrottleMinHeadroom','WindConfidenceMean','AbruptGustTriggerCount','EnergyRecoveryActiveFraction','EnergyAltitudeShiftPeak_m','InputSnapMax','DisturbanceEvidenceActiveFraction','ForcedResponseRms','ForcedResponsePeak','ReleaseWindEvidenceActiveFraction','CarrierTransientEvidenceActiveFraction','CarrierTransientSwitchCount','WindMpcSwitchCount','LastCarrierTransientTime_s','LastWindMpcActiveTime_s','Tail5BaseSolverFraction','Tail5ZeroTruthWindFraction','ZeroWindSettleDuration_s','StateEstP95Norm','PositionEstP95_m','GroundspeedEstP95_mps','VerticalSpeedEstP95_mps','PaperSensorModel','IndependentCargoTruth'});
...
if height(comparison)>0
    comparison.CarrierRmsImprovement_pct=100*(1-1./comparison.LegacyOverWindMpcCarrierRms);
    comparison.ReleaseRmsImprovement_pct=100*(1-1./comparison.NoWindOverAwareLandingRms);
end
passIntegrated=height(A)==height(M) && all(A.Found) && all(A.PaperCorePass);
...
R=struct(); R.version="Physics-MPC v1.3.6-Paper fair-sensor MPC validation"; R.pass=passIntegrated && allExecuted; R.integrated_pass=passIntegrated; R.all_three_way_executed=allExecuted; R.rows=rows; R.comparison=comparison; R.median_legacy_over_wind_mpc_carrier_rms=medianCarrierRatio; R.completed_at=datetime("now");
...
writetable(rows,fullfile(root,"paper_validation.csv")); writetable(comparison,fullfile(root,"paper_comparison.csv")); save(fullfile(root,"paper_validation.mat"),"R","-v7.3");
localWrite(R,fullfile(root,"paper_validation_summary.txt")); localPlot(comparison,root);
end
function localWrite(R,path)
fid=fopen(path,"w"); if fid<0, return; end; c=onCleanup(@()fclose(fid)); %#ok<NASGU>
fprintf(fid,"Physics-MPC v1.3.6-Paper fair-sensor MPC validation\npaper_validation_integrity_pass=%d\npaper_core_wind_mpc_scenarios_pass=%d/8\nall_three_way_missions_found=%d/24\nengineering_wind_mpc_scenarios_pass=%d/8\n",R.pass,sum(R.rows.PaperCorePass & R.rows.Mode=="wind_mpc_aware"),sum(R.rows.Found),sum(R.rows.EngineeringPass & R.rows.Mode=="wind_mpc_aware"));
...
fprintf(fid,"wind_mpc_worst_state_est_p95_norm=%.9g\nwind_mpc_worst_position_est_p95_m=%.9g\nwind_mpc_worst_groundspeed_est_p95_mps=%.9g\nwind_mpc_worst_verticalspeed_est_p95_mps=%.9g\n",max(A.StateEstP95Norm,[],'omitnan'),max(A.PositionEstP95_m,[],'omitnan'),max(A.GroundspeedEstP95_mps,[],'omitnan'),max(A.VerticalSpeedEstP95_mps,[],'omitnan'));
...
try
    ...
    nexttile; bar(categorical(C.Scenario),[C.WindMpcPostWindRms C.LegacyPostWindRms]); ylabel("post-wind primary RMS"); legend("paper wind-aware MPC","legacy MPC"); grid on;
    nexttile; bar(categorical(C.Scenario),[C.WindMpcRecovery_s C.LegacyRecovery_s]); ylabel("gust recovery (s)"); legend("paper wind-aware MPC","legacy MPC"); grid on;
    ...
    exportgraphics(f,fullfile(root,"paper_validation.png"),"Resolution",160); close(f);
catch ME
