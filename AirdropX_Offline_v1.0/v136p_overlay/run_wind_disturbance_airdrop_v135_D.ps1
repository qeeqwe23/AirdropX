...
$out=Join-Path $ProjectRoot 'matlab\results\physics_mpc_v135_equivalent_energy_recovery_airdrop_validation'; New-Item -ItemType Directory -Force -Path $out|Out-Null
...
$eqRoot=Join-Path $ProjectRoot 'matlab\results\physics_mpc_v135_base_equivalence_audit'; $eqEsc=E $eqRoot
$cmd="cd('$rootEsc'); addpath('matlab'); addpath('matlab/phys_mpc'); addpath('matlab/wind'); addpath('matlab/airdrop'); R=airdropx_phys_mpc_base_equivalence_audit_v135(string(pwd),OutputRoot=string('$eqEsc'),WindCalibrationPath=string('$calEsc'),SensorNoiseSeed=101); disp(R); assert(R.pass);"
$code=Invoke-MatlabCapture $cmd (Join-Path $out 'base_equivalence_terminal.txt'); if($code -ne 0){throw 'v1.3.5 base-equivalence audit failed. Full suite intentionally not started.'}
$jobs=@()
...
$cmd="cd('$rootEsc'); addpath('matlab'); addpath('matlab/airdrop'); R=airdropx_wind_airdrop_finalize_v135(string('$outEsc'),string('$manifestEsc')); disp(R);"
$code=Invoke-MatlabCapture $cmd (Join-Path $out 'finalize_terminal.txt'); if($code -ne 0){throw 'v1.3.5 energy-recovery finalizer failed.'}
Get-Content (Join-Path $out 'wind_equivalent_energy_recovery_validation_summary.txt')
